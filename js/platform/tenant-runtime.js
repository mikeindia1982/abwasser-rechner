...
const TENANT_SNAPSHOT_PREFIX = "abwasser-tenant-snapshot-v2:";
...
const APP_LOCAL_STORAGE_KEYS = Object.freeze([
  ...
  "abwasser-reverse-geocode-v01",
  "abwasser-sales-reminder-notice-v01",
  "vta-google-maps-api-key-v01",
  "vta-google-maps-map-id-v01",
  "vta-workspace-mode-v01",
  "vta-firebase-session-cache-v01",
  "vta-firebase-last-email-v01",
]);
...
function resolveRequestedTenantId() {
  ...
  if (listTenantConfigs().some((tenant) => tenant.id === fromUrl)) return fromUrl;

  const stored = localStorage.getItem(ACTIVE_TENANT_KEY);
  if (listTenantConfigs().some((tenant) => tenant.id === stored)) return stored;
...
function applyTenantBranding(config = activeTenant) {
  ...
  if (manifest) {
    manifest.setAttribute(
      "href",
      config.id === "platform" ? "manifest-platform.webmanifest" : "manifest-vta.webmanifest",
    );
  }
