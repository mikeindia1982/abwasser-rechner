...
function plants() {
  const rows = readJson('abwasser-plants-v07', []);
  return Array.isArray(rows) ? rows : [];
}

function products() {
  const rows = readJson('abwasser-products-v092', []);
  return Array.isArray(rows) ? rows : [];
}

function activePlantId() {
  return localStorage.getItem('abwasser-active-plant-v07') || '';
}

function plantLabel(plant) {
  return plant?.master?.name || plant?.name || plant?.title || plant?.operator?.name || 'Unbenannte Anlage';
}

function productLabel(product) {
  return product?.name || product?.title || product?.productName || 'Unbenanntes Produkt';
}

function linkedPlant(entry) {
  return entry.links?.find((link) => link.entityType === 'plant') || null;
}

function linkedProduct(entry) {
  return entry.links?.find((link) => link.entityType === 'product') || null;
}
...
function matches(entry) {
  ...
  const haystack = [
    ...
    ...(entry.tags || []),
    ...(entry.links || []).flatMap((link) => [link.entityLabel, link.entityType, link.relationType]),
...
function levelOptions(selected = '') {
  return Object.entries(LEVEL_LABELS)
    .map(([value, label]) => `<option value="${value}" ${value === selected ? 'selected' : ''}>${esc(label)}</option>`)
    .join('');
}
  ...
  return `
    <form id="knowledgeForm" class="knowledge-form">
      <div class="knowledge-form-head">
        <div>
          <p class="eyebrow">${entry.id ? 'Wissenseintrag bearbeiten' : 'Neues Wissen'}</p>
          <h2>${entry.id ? esc(entry.title || 'Wissenseintrag') : 'Wissenseintrag anlegen'}</h2>
        </div>
        <button class="button secondary" type="button" data-kb-action="cancel-form">Schließen</button>
      </div>
      <div class="knowledge-form-grid">
        ...
        <label>Wissenstyp<select name="knowledgeType" id="knowledgeTypeSelect">${typeOptions(selectedType)}</select></label>
        <label>Status<select name="status">${statusOptions(entry.status || 'draft')}</select></label>
        <label>Wissensqualität<select name="knowledgeLevel">${levelOptions(entry.knowledgeLevel || 'unverified')}</select></label>
        <label>Sichtbarkeit<select name="visibility">${Object.entries(VISIBILITY_LABELS).map(([value, label]) => `<option value="${value}" ${value === (entry.visibility || 'company') ? 'selected' : ''}>${esc(label)}</option>`).join('')}</select></label>
      ...
      <div class="knowledge-form-actions">
        <button class="button secondary" type="button" data-kb-action="cancel-form">Abbrechen</button>
        <button class="button primary" type="submit">Wissenseintrag speichern</button>
      </div>
    </form>`;
}

function cardMarkup(entry) {
  const plant = linkedPlant(entry);
  const product = linkedProduct(entry);
  return `
    <article class="knowledge-card" data-kb-id="${esc(entry.id)}" tabindex="0">
      <div class="knowledge-card-top">
        <span class="knowledge-type">${esc(TYPE_LABELS[entry.knowledgeType] || entry.knowledgeType)}</span>
        <span class="knowledge-status" data-status="${esc(entry.status)}">${esc(STATUS_LABELS[entry.status] || entry.status)}</span>
      </div>
      <h3>${esc(entry.title)}</h3>
      <p>${esc(entry.summary || entry.content || 'Noch keine Zusammenfassung hinterlegt.')}</p>
      <div class="knowledge-meta">
        <span>${esc(LEVEL_LABELS[entry.knowledgeLevel] || entry.knowledgeLevel)}</span>
        ${plant ? `<span>Anlage: ${esc(plant.entityLabel || plant.entityId)}</span>` : ''}
        ${product ? `<span>Produkt: ${esc(product.entityLabel || product.entityId)}</span>` : ''}
      </div>
...
function detailMarkup(entry) {
  ...
  return `
    <div class="knowledge-detail-head">
      <div>
        <p class="eyebrow">${esc(TYPE_LABELS[entry.knowledgeType] || entry.knowledgeType)}</p>
        <h2>${esc(entry.title)}</h2>
        <div class="knowledge-detail-badges"><span>${esc(STATUS_LABELS[entry.status] || entry.status)}</span><span>${esc(LEVEL_LABELS[entry.knowledgeLevel] || entry.knowledgeLevel)}</span><span>${esc(VISIBILITY_LABELS[entry.visibility] || entry.visibility)}</span></div>
      </div>
      <div class="knowledge-detail-actions">
        <button class="button secondary" type="button" data-kb-action="edit" data-kb-id="${esc(entry.id)}">Bearbeiten</button>
        ${entry.status !== 'archived' ? `<button class="button secondary" type="button" data-kb-action="archive" data-kb-id="${esc(entry.id)}">Archivieren</button>` : ''}
      </div>
    </div>
    ${entry.summary ? `<section><h3>Zusammenfassung</h3><p>${esc(entry.summary)}</p></section>` : ''}
    ${entry.content ? `<section><h3>Beschreibung</h3><p class="knowledge-prewrap">${esc(entry.content)}</p></section>` : ''}
    ${fieldRows.map(([key, value]) => `<section><h3>${esc(fieldLabels[key] || key)}</h3><p class="knowledge-prewrap">${esc(value)}</p></section>`).join('')}
...
async function render(root) {
  ...
  const plantRows = plants();
  const activeId = activePlantId();

  root.innerHTML = `
    <div class="knowledge-page">
      <div class="knowledge-hero">
        <div>
          <p class="eyebrow">Unternehmensgedächtnis</p>
          <h1>Wissensdatenbank</h1>
          <p>Fachwissen, Praxiserfahrungen, Problemlösungen und Quellen strukturiert dokumentieren und mit Anlagen oder Produkten verknüpfen.</p>
        </div>
        <div class="knowledge-hero-actions">
          ${activeId ? `<button class="button secondary" type="button" data-kb-action="active-plant">Wissen der aktiven Anlage</button>` : ''}
          <button class="button primary" type="button" data-kb-action="new">＋ Neuer Wissenseintrag</button>
        </div>
      </div>
      <div class="knowledge-kpis">
        <div><strong>${entries.filter((entry) => entry.status !== 'archived').length}</strong><span>aktive Einträge</span></div>
        <div><strong>${entries.filter((entry) => entry.status === 'approved').length}</strong><span>freigegeben</span></div>
        <div><strong>${entries.filter((entry) => ['practical','verified','official'].includes(entry.knowledgeLevel)).length}</strong><span>qualifiziert</span></div>
        <div><strong>${new Set(entries.flatMap((entry) => entry.tags || [])).size}</strong><span>Tags</span></div>
      </div>
      <div class="knowledge-toolbar">
        <input id="knowledgeSearch" type="search" placeholder="Wissen durchsuchen …" value="${esc(state.query)}">
        <select id="knowledgeTypeFilter"><option value="all">Alle Wissenstypen</option>${Object.entries(TYPE_LABELS).map(([value, label]) => `<option value="${value}" ${state.type === value ? 'selected' : ''}>${esc(label)}</option>`).join('')}</select>
        <select id="knowledgeStatusFilter"><option value="active" ${state.status === 'active' ? 'selected' : ''}>Aktiv</option><option value="all" ${state.status === 'all' ? 'selected' : ''}>Alle Status</option>${Object.entries(STATUS_LABELS).map(([value, label]) => `<option value="${value}" ${state.status === value ? 'selected' : ''}>${esc(label)}</option>`).join('')}</select>
        <select id="knowledgeLevelFilter"><option value="all">Alle Qualitätsstufen</option>${Object.entries(LEVEL_LABELS).map(([value, label]) => `<option value="${value}" ${state.level === value ? 'selected' : ''}>${esc(label)}</option>`).join('')}</select>
        <select id="knowledgePlantFilter"><option value="all">Alle Anlagen</option>${plantRows.map((plant) => `<option value="${esc(plant.id)}" ${String(state.plant) === String(plant.id) ? 'selected' : ''}>${esc(plantLabel(plant))}</option>`).join('')}</select>
      </div>
      <div class="knowledge-layout">
        <div class="knowledge-list">
          <div class="knowledge-list-head"><strong>${filtered.length} Treffer</strong><span>${entries.length} Einträge gesamt</span></div>
          <div class="knowledge-cards">${filtered.length ? filtered.map(cardMarkup).join('') : '<div class="knowledge-empty"><h3>Keine Treffer</h3><p>Filter anpassen oder einen neuen Wissenseintrag anlegen.</p></div>'}</div>
        </div>
        <aside class="knowledge-detail">${detailMarkup(selected)}</aside>
      </div>
...
function openForm(root, entry = {}) {
  const modal = root.querySelector('#knowledgeModal');
  const card = modal.querySelector('.knowledge-modal-card');
  state.editingId = entry.id || null;
  card.innerHTML = formMarkup(entry);
  modal.classList.remove('hidden');
  modal.setAttribute('aria-hidden', 'false');

  const typeSelect = card.querySelector('#knowledgeTypeSelect');
  typeSelect?.addEventListener('change', () => {
    card.querySelector('#knowledgeDynamicFields').innerHTML = dynamicFields(typeSelect.value, {});
  });
  ...
  card.querySelector('#knowledgeForm')?.addEventListener('submit', async (event) => {
    ...
    const links = [];
    if (plantId) {
      const plant = plantRows.find((row) => String(row.id) === String(plantId));
      links.push({ entityType: 'plant', entityId: String(plantId), entityLabel: plantLabel(plant), relationType: 'observed_at' });
    }
    if (productId) {
      const product = productRows.find((row) => String(row.id) === String(productId));
      links.push({ entityType: 'product', entityId: String(productId), entityLabel: productLabel(product), relationType: 'related_to' });
    }
    const sourceTitle = String(formData.get('sourceTitle') || '').trim();
    const sources = sourceTitle ? [{ sourceType: formData.get('sourceType') || 'manual', sourceTitle }] : (current?.sources || []);
    const saved = await knowledgeRepository.save({
      ...current,
...
function bind(root, entries) {
  ...
  root.querySelector('#knowledgeSearch')?.addEventListener('input', (event) => {
    state.query = event.target.value;
    clearTimeout(state.searchTimer);
    state.searchTimer = setTimeout(async () => {
      await render(root);
      const input = root.querySelector('#knowledgeSearch');
      if (input) {
        input.focus();
  ...
  root.querySelectorAll('.knowledge-card').forEach((card) => {
    const select = () => { state.selectedId = card.dataset.kbId; rerender(); };
    card.addEventListener('click', select);
    card.addEventListener('keydown', (event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); select(); } });
  });

  root.querySelectorAll('[data-kb-action]').forEach((button) => {
    button.addEventListener('click', async () => {
      ...
      if (action === 'edit') {
        const entry = entries.find((row) => row.id === button.dataset.kbId);
        if (entry) return openForm(root, entry);
      }
      if (action === 'archive') {
        await knowledgeRepository.archive(button.dataset.kbId);
        return rerender();
      }
    });
  });
