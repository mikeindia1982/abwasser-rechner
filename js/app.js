...
function saveDocuments(){try{const payload=JSON.stringify(documents);localStorage.setItem(STORAGE_DOCUMENTS,payload);if(localStorage.getItem(STORAGE_DOCUMENTS)!==payload)throw new Error("Speicherprüfung fehlgeschlagen");return true}catch(e){console.error(e);alert("Dokumentmetadaten konnten nicht gespeichert werden. Bitte Browser-Speicher prüfen.");return false}}
