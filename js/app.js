...
const categoryMeta = {
  Phosphor: {
    icon: "P",
    description: "Fällmittelbedarf, molare Stoffdaten und Handelsprodukte",
  },
  Schlammentwässerung: {
    icon: "TS",
    description: "Durchsatz, Kuchenmenge, Polymer und Wirtschaftlichkeit",
  },
  Grundlagen: {
    icon: "∑",
    description: "Frachten, Konzentrationen und grundlegende Umrechnungen",
  },
  Biologie: {
    icon: "BIO",
    description: "Schlammalter, Schlammbelastung und biologische Kennwerte",
  },
  Schlamm: { icon: "SVI", description: "Schlammkennwerte und Absetzverhalten" },
  Hydraulik: {
    icon: "Q",
    description: "Volumen, Durchfluss und Aufenthaltszeiten",
  },
  Chemikalien: {
    icon: "CH",
    description: "Dosierströme, Bestände und Chemikalienverbrauch",
  },
  Wirtschaftlichkeit: {
    icon: "€",
    description: "Kosten, Vergleiche und Einsparpotenziale",
  },
};
...
function combinePhone(formData, prefix) {
  ...
  const number = (formData.get(`${prefix}.number`) || "")
    .trim()
    .replace(/\s+/g, " ");
...
function exportVisitIcs(plant, visit) {
  ...
  const end = isoLocalToDate(visit.end);
  if (!start || !end)
    return alert("Bitte Start- und Endzeit vollständig hinterlegen.");
  ...
  const safe = (plant.master.name || "klaeranlage")
    .toLowerCase()
...
function normalizeVisit(value = {}) {
  ...
  return {
    id: source.id || makeId(),
    title: source.title || "Besuch",
    start: source.start || "",
    end: source.end || "",
    purpose: source.purpose || "",
    contact: source.contact || "",
    visitType: source.visitType || "process-optimization",
    processArea: source.processArea || "",
    objective: source.objective || "",
    initialSituation: source.initialSituation || "",
    workPerformed: source.workPerformed || "",
    chemistryChanges: source.chemistryChanges || "",
    settingChanges: source.settingChanges || "",
    result: source.result || "",
    recommendation: source.recommendation || "",
    nextSteps: source.nextSteps || "",
    comparison: {
      beforeProduct: source.comparison?.beforeProduct || "",
      afterProduct: source.comparison?.afterProduct || "",
      beforeDose: source.comparison?.beforeDose || "",
      afterDose: source.comparison?.afterDose || "",
      beforeCost: source.comparison?.beforeCost || "",
      afterCost: source.comparison?.afterCost || "",
      beforeQuality: source.comparison?.beforeQuality || "",
      afterQuality: source.comparison?.afterQuality || "",
    },
    status: source.status || "planned",
    notes: source.notes || "",
    modeStatus: source.modeStatus || "not-started",
    startedAt: source.startedAt || "",
    completedAt: source.completedAt || "",
    checklist: {
      ...Object.fromEntries(VISIT_CHECKLIST.map(([key]) => [key, false])),
      ...(source.checklist || {}),
    },
    measurements: {
      flow: source.measurements?.flow || "",
      pOut: source.measurements?.pOut || "",
      nh4Out: source.measurements?.nh4Out || "",
      cakeTs: source.measurements?.cakeTs || "",
      polymer: source.measurements?.polymer || "",
      custom: source.measurements?.custom || "",
    },
...
function normalizePlant(value = {}) {
  ...
  const normalized = {
    ...
    dosingSystems: Array.isArray(source.dosingSystems)
      ? source.dosingSystems.map(dosingDefaults)
      : [],
    tankSystems: Array.isArray(source.tankSystems)
      ? source.tankSystems.map(tankDefaults)
      : [],
    contacts: Array.isArray(source.contacts) ? source.contacts : [],
    visits: Array.isArray(source.visits)
      ? source.visits.map(normalizeVisit)
      : [],
    actions: Array.isArray(source.actions)
      ? source.actions.map((a) => ({
          id: a.id || makeId(),
          title: a.title || "Aufgabe",
          status: a.status || "open",
          priority: a.priority || "normal",
          dueDate: a.dueDate || "",
          component: a.component || "",
          sourceVisitId: a.sourceVisitId || "",
          createdAt: a.createdAt || new Date().toISOString(),
          completedAt: a.completedAt || "",
        }))
      : [],
    limits:
      Array.isArray(source.limits) && source.limits.length
        ? source.limits
        : structuredClone(defaultLimits),
  };
  normalized.master.processStages = Array.isArray(
    normalized.master.processStages,
  )
    ? normalized.master.processStages
    : [];
  ...
  normalized.address.longitude =
    normalized.address.longitude || legacy.longitude;
  return normalized;
}
...
function loadEmployeeProfile() {
  try {
    return normalizeEmployeeProfile(
      JSON.parse(localStorage.getItem(STORAGE_PROFILE) || "{}"),
    );
  } catch {
...
function saveEmployeeProfile() {
  try {
    const payload = JSON.stringify(employeeProfile);
    localStorage.setItem(STORAGE_PROFILE, payload);
    if (localStorage.getItem(STORAGE_PROFILE) !== payload)
      throw new Error("Speicherprüfung fehlgeschlagen");
    updateProfileButton();
    return true;
  } catch (error) {
    console.error(error);
    alert("Das Mitarbeiterprofil konnte nicht gespeichert werden.");
    return false;
  }
}
...
function updateProfileButton() {
  const name = document.querySelector("#profileButtonName"),
...
function employeeVCard() {
  ...
  return [
    ...
    p.jobTitle ? `TITLE:${escapeVCard(p.jobTitle)}` : "",
    p.mobile ? `TEL;TYPE=CELL:${escapeVCard(p.mobile)}` : "",
    p.phone ? `TEL;TYPE=WORK:${escapeVCard(p.phone)}` : "",
    p.email ? `EMAIL;TYPE=INTERNET,WORK:${escapeVCard(p.email)}` : "",
    p.website ? `URL:${escapeVCard(p.website)}` : "",
    p.street || p.city
      ? `ADR;TYPE=WORK:;;${escapeVCard(p.street)};${escapeVCard(p.city)};;${escapeVCard(p.postalCode)};${escapeVCard(p.country)}`
      : "",
    "END:VCARD",
  ]
    .filter(Boolean)
...
function qrSvg(text, size = 220) {
  try {
    const qr = new window.QRCodeGenerator(0, window.QRErrorCorrectLevel.M);
    qr.addData(text);
    qr.make();
    ...
    for (let r = 0; r < n; r++)
      for (let c = 0; c < n; c++)
        if (qr.isDark(r, c))
          rects += `<rect x="${((c + quiet) * cell).toFixed(2)}" y="${((r + quiet) * cell).toFixed(2)}" width="${(cell + 0.15).toFixed(2)}" height="${(cell + 0.15).toFixed(2)}"/>`;
    return `<svg viewBox="0 0 ${size} ${size}" role="img" aria-label="QR-Code mit Kontaktdaten"><rect width="100%" height="100%" fill="white"/><g fill="black">${rects}</g></svg>`;
  } catch (error) {
    console.error(error);
    return `<div class="qr-error">QR-Code konnte nicht erzeugt werden.</div>`;
  }
}
function downloadVCard() {
  const blob = new Blob([employeeVCard()], {
...
const seededProducts = [
  {
    id: "product-aquafix-70-plus",
    name: "VTA Aquafix® 70 plus",
    materialNumber: "33",
    category: "Fällungs- und Flockungsmittel",
    status: "active",
    shortDescription:
      "Flüssiges Fällungs- und Flockungsmittel in wässriger Lösung.",
    applications: ["Fällung", "Flockung"],
    problems: [],
    benefits: [],
    technical: {
      state: "flüssig",
      ...
      ph: "< 2",
      density: "ca. 1,3 g/cm³",
      solubility: "vollständig mischbar",
      storageStability: "12 Monate",
    },
    safety: {
      signalWord: "Gefahr",
      hazardStatements: ["H290", "H318"],
      unNumber: "UN1760",
      transportClass: "8",
      waterHazardClass: "1",
    },
  ...
  },
  {
    ...
    name: "VTA Biokat®",
    materialNumber: "",
    category: "Biologische Prozessunterstützung",
    status: "active",
    shortDescription:
      "Maßgeschneiderte Bio-Kost zur Stabilisierung und Aktivierung der biologischen Reinigungsleistung.",
    applications: ["Biologische Abwasserreinigung", "Belebungsanlage"],
    problems: [
      "Blähschlamm",
      "Schwimmschlamm",
      "starke Fädigkeit",
      "lockere und instabile Flocken",
      "gestörte Reinigungsleistung",
    ],
    benefits: [
      "Verbessert Reinigungsleistung und Schlammeigenschaften",
      "Kompakte und stabile Flocken",
      "Reduziert Energie- und Produktverbrauch",
...
function loadProducts() {
  try {
    ...
    if (!raw) {
      localStorage.setItem(STORAGE_PRODUCTS, JSON.stringify(seededProducts));
      return seededProducts.map(normalizeProduct);
    }
    const data = JSON.parse(raw);
    return Array.isArray(data)
      ? data.map(normalizeProduct)
      : seededProducts.map(normalizeProduct);
  } catch {
    return seededProducts.map(normalizeProduct);
  }
}
...
function saveProducts() {
  try {
    ...
    console.error(error);
    alert("Produktdaten konnten nicht gespeichert werden.");
    return false;
  }
}
...
function detectDocumentType(text, name = "") {
  const hay = `${name}\n${text}`.toLowerCase();
  if (
    hay.includes("sicherheitsdatenblatt") ||
    hay.includes("abschnitt 1:") ||
    hay.includes("verordnung (eg) nr. 1907/2006")
  )
    return "sds";
  if (
    hay.includes("factsheet") ||
    hay.includes("eine bio-innovation") ||
    hay.includes("viele vorteile") ||
    hay.includes("100% effizient")
  )
    return "factsheet";
  if (
    hay.includes("technisches merkblatt") ||
    hay.includes("technische information")
  )
    return "technical";
  ...
  return (
    {
      ...
      factsheet: "Factsheet",
      "product-sheet": "Produktdatenblatt",
      technical: "Technisches Merkblatt",
      other: "Sonstiges Dokument",
    }[type] || "Sonstiges Dokument"
  );
}
...
async function extractPdfTextBasic(file) {
  ...
  const literal = [...decoded.matchAll(/\(([^()]*(?:\\.[^()]*)*)\)\s*Tj/g)].map(
    (m) => m[1],
  );
  const arrays = [...decoded.matchAll(/\[(.*?)\]\s*TJ/gs)].flatMap((m) =>
...
function inferProductFromPdf(text, fileName, type) {
  ...
  if (vta)
    name = vta[0]
      .replace(/(?:_D-de|Sicherheitsdatenblatt|Factsheet).*$/i, "")
      .trim();
  if (!name)
    name = fileName
      .replace(/\.pdf$/i, "")
  ...
  const date =
    (hay.match(
      /(?:Überarbeitet am|Stand|Version)\s*:?\s*(\d{2}\.\d{2}\.\d{4})/i,
    ) || [])[1] || "";
  const category =
    type === "sds"
      ? hay.match(/Fällungsmittel[^,.;]*,?\s*Flockungsmittel/i)
        ? "Fällungs- und Flockungsmittel"
        : "Chemisches Produkt"
      : /mikroorganismen|bio-kost|biologisch/i.test(hay)
        ? "Biologische Prozessunterstützung"
        : "Sonstiges";
  const result = {
    name,
  ...
  if (type === "sds") {
    result.applications = unique([
      /Fällungsmittel/i.test(hay) ? "Fällung" : "",
      /Flockungsmittel/i.test(hay) ? "Flockung" : "",
    ]);
    result.technical.state =
      (hay.match(/Aggregatzustand\s*:?\s*(flüssig|fest|gasförmig)/i) ||
        [])[1] || "";
    result.technical.ph =
      (hay.match(
        /pH-Wert[^:<]{0,30}:?\s*(<\s*\d+(?:[,.]\d+)?|\d+(?:[,.]\d+)?(?:\s*[-–]\s*\d+(?:[,.]\d+)?)?)/i,
      ) || [])[1] || "";
    result.technical.density =
      (hay.match(/Dichte\s*:?\s*(?:ca\.\s*)?([\d,.]+\s*g\/cm³)/i) || [])[1] ||
      "";
    result.safety.signalWord =
      (hay.match(/Signalwort\s*:?\s*(Gefahr|Achtung)/i) || [])[1] || "";
    result.safety.hazardStatements = unique(
      [...hay.matchAll(/\bH(\d{3})\b/g)].map((m) => `H${m[1]}`),
    ).slice(0, 12);
    result.safety.unNumber =
      (hay.match(/\bUN\s?(\d{4})\b/i) || [])[0]?.replace(/\s/g, "") || "";
  } else if (type === "factsheet") {
    const mappings = [
      ["Blähschlamm", /blähschlamm/i],
      ["Schwimmschlamm", /schwimmschlamm/i],
      ["starke Fädigkeit", /starke fädigkeit/i],
      ["lockere und instabile Flocken", /lockere.{0,5}instabile flocken/i],
    ];
    result.problems = mappings.filter(([, re]) => re.test(hay)).map(([v]) => v);
    result.benefits = unique([
      /verbessert.{0,80}reinigungsleistung/i.test(hay)
        ? "Verbessert die Reinigungsleistung"
        : "",
      /verbessert.{0,100}schlammeigenschaften/i.test(hay)
        ? "Verbessert die Schlammeigenschaften"
        : "",
      /reduziert.{0,80}energie/i.test(hay)
        ? "Reduziert Energie- und Produktverbrauch"
        : "",
      /biologisch verträglich/i.test(hay) ? "Biologisch verträglich" : "",
    ]);
    result.applications = /kläranlage|abwasserreinigung/i.test(hay)
...
function productImportQuality(type, inferred, rawText) {
  ...
  if (type === "sds")
    checks.push(
      [
        "Technische Daten erkannt",
        Boolean(
          inferred.technical.state ||
          inferred.technical.ph ||
          inferred.technical.density,
        ),
      ],
      [
        "Gefahrendaten erkannt",
        Boolean(
          inferred.safety.signalWord ||
          (inferred.safety.hazardStatements || []).length ||
          inferred.safety.unNumber,
        ),
      ],
    );
  if (type === "factsheet")
    checks.push(
      ["Anwendungen erkannt", Boolean(inferred.applications.length)],
      ["Probleme erkannt", Boolean(inferred.problems.length)],
      ["Nutzen erkannt", Boolean(inferred.benefits.length)],
    );
...
function importStatusLabel(status) {
  return (
    {
      queued: "Wartet",
      reading: "PDF wird eingelesen",
      analyzing: "Wissensanalyse läuft",
...
function renderProductImportQueueStatus() {
  const host = $("#productImportStatus");
  if (!host) return;
  const active = productImportQueue.filter((x) => x.status !== "saved");
  if (!productImportQueue.length) {
    host.innerHTML = "";
    return;
  }
  const completed = productImportQueue.filter((x) =>
    ["ready", "saved", "error"].includes(x.status),
  ).length;
  ...
  $$("[data-review-import]").forEach(
    (b) =>
      (b.onclick = () => {
        const item = productImportQueue.find(
          (x) => x.id === b.dataset.reviewImport,
        );
        if (item)
          showProductImportReview(
            item.file,
            item.type,
            item.inferred,
            item.rawText,
  ...
  const clear = $("#clearImportQueue");
  if (clear)
    clear.onclick = () => {
      productImportQueue = [];
      renderProductImportQueueStatus();
    };
}
async function processProductImportQueue() {
  if (productImportProcessing) return;
  productImportProcessing = true;
  while (true) {
    const item = productImportQueue.find((x) => x.status === "queued");
    if (!item) break;
    try {
      item.status = "reading";
      renderProductImportQueueStatus();
      await new Promise((r) => setTimeout(r, 30));
      item.tempFileId = item.tempFileId || `import-${item.id}`;
      await storeProductFile(item.tempFileId, item.file);
      item.rawText = await extractPdfTextBasic(item.file);
      item.status = "analyzing";
      renderProductImportQueueStatus();
      await new Promise((r) => setTimeout(r, 60));
      item.type = detectDocumentType(item.rawText, item.file.name);
      item.inferred = inferProductFromPdf(
        item.rawText,
        item.file.name,
        item.type,
      );
      item.quality = productImportQuality(
        item.type,
        item.inferred,
        item.rawText,
      );
...
function enqueueProductPdfs(files) {
  ...
  renderProductImportQueueStatus();
  processProductImportQueue();
}

function openProductDb() {
  return new Promise((resolve, reject) => {
    const req = indexedDB.open("abwasser-product-documents-v1", 1);
    req.onupgradeneeded = () => {
...
async function storeProductFile(key, file) {
  const db = await openProductDb();
  const record = {
    blob:
      file instanceof Blob
        ? file
        : new Blob([file], { type: file?.type || "application/pdf" }),
...
async function deleteProductFile(key) {
  const db = await openProductDb();
  return new Promise((resolve, reject) => {
    const tx = db.transaction("files", "readwrite");
...
let activePlantId =
  localStorage.getItem(STORAGE_ACTIVE_PLANT) || plants[0]?.id || "";
if (activePlantId && !plants.some((p) => p.id === activePlantId))
...
const state = {
  ...
  favoritesOnly: false,
  favorites: new Set(
    JSON.parse(localStorage.getItem(STORAGE_FAVORITES) || "[]"),
  ),
  openCategories: new Set(
    JSON.parse(localStorage.getItem(STORAGE_MENU) || "[]"),
  ),
...
function savePlants() {
  try {
    ...
    if (localStorage.getItem(STORAGE_PLANTS) !== payload)
      throw new Error("Speicherprüfung fehlgeschlagen");
    if (activePlantId)
      localStorage.setItem(STORAGE_ACTIVE_PLANT, activePlantId);
    ...
    alert(
      "Die Anlagendaten konnten im Browser nicht gespeichert werden. Bitte freien Speicher prüfen und die Seite nicht schließen.",
    );
    return false;
  }
}
...
function processStageLabels(values) {
  return (Array.isArray(values) ? values : []).map(
    (value) => processStageOptions.find(([key]) => key === value)?.[1] || value,
  );
}
...
function telLink(value = "") {
  ...
  return tel
    ? `<a class="contact-link" href="tel:${tel}">${esc(value)}</a>`
    : "–";
}
...
function nextInternalNumber() {
  const maximum = plants.reduce((max, plant) => {
    const match = String(plant.master?.internalNumber || "").match(
      /ANL-(\d+)/i,
    );
...
function procedureConfig(plant) {
  ...
  const stages = new Set(
    Array.isArray(plant?.master?.processStages)
      ? plant.master.processStages
      : [],
  );
  return {
    main,
    ...
    precipitation:
      ...
      (plant?.dosingSystems || []).some((x) => x.purpose === "precipitant"),
    digestion: stages.has("sludge-digestion"),
    dewatering:
      stages.has("sludge-dewatering") ||
      Boolean(plant?.sludgeDewatering?.enabled),
    filtration: [
      "sand-filtration",
      "cloth-filtration",
      "disc-filtration",
...
function procedureSvg(plant, { interactive = true } = {}) {
  ...
  if (c.primary)
    flow.push({ id: "primary", label: "Vorklärung", kind: "clarifier" });
  flow.push({ id: "biology", label: biological[0], kind: biological[1] });
  if (
    c.main !== "sbr" &&
    c.main !== "mbr" &&
    c.main !== "constructed-wetland" &&
    c.main !== "lagoon"
  )
    flow.push({ id: "secondary", label: "Nachklärung", kind: "clarifier" });
  if (c.filtration)
    flow.push({
      id: "filtration",
      label: "Weitergehende Reinigung",
      kind: "filter",
    });
  ...
  const nodes = flow
    .map((n, i) => {
      ...
      let shape = "";
      if (n.kind === "clarifier")
        shape = `<circle cx="${x}" cy="${y}" r="43" class="proc-water"/><g class="proc-rotor"><line x1="${x}" y1="${y}" x2="${x + 36}" y2="${y}"/><circle cx="${x}" cy="${y}" r="5"/></g>`;
      else if (n.kind === "trickling")
        shape = `<circle cx="${x}" cy="${y}" r="43" class="proc-media"/><g class="proc-rotor"><line x1="${x - 34}" y1="${y}" x2="${x + 34}" y2="${y}"/><circle cx="${x}" cy="${y}" r="5"/></g>`;
      else if (n.kind === "outlet" || n.kind === "inlet")
        shape = `<rect x="${x - 42}" y="${y - 29}" width="84" height="58" rx="18" class="proc-${n.kind}"/>`;
      else
        shape = `<rect x="${x - 55}" y="${y - 43}" width="110" height="86" rx="22" class="proc-water"/>${["activated", "sbr", "mbr"].includes(n.kind) ? `<g class="proc-bubbles">${[-30, -10, 12, 31].map((dx, j) => `<circle cx="${x + dx}" cy="${y + 24 - (j % 2) * 8}" r="3"/>`).join("")}</g>` : ""}${n.kind === "mbr" ? `<g class="proc-membrane"><line x1="${x - 18}" y1="${y - 25}" x2="${x - 18}" y2="${y + 25}"/><line x1="${x}" y1="${y - 25}" x2="${x}" y2="${y + 25}"/><line x1="${x + 18}" y1="${y - 25}" x2="${x + 18}" y2="${y + 25}"/></g>` : ""}`;
  ...
  const pipes = flow
    .slice(0, -1)
    .map((_, i) => {
      const x1 = start + i * gap + 55,
        x2 = start + (i + 1) * gap - 55;
      return `<line x1="${x1}" y1="${y}" x2="${x2}" y2="${y}" class="proc-pipe"/>`;
    })
    .join("");
  const extras = [];
  if (c.precipitation) {
    const x =
      ...
      Math.max(
        1,
        flow.findIndex((n) => n.id === "biology"),
      ) *
        gap;
    extras.push(
      `<g class="procedure-node ${interactive ? "is-interactive" : ""}" data-procedure-node="precipitation" tabindex="${interactive ? "0" : "-1"}"><rect x="${x - 38}" y="12" width="76" height="48" rx="15" class="proc-dose"/><path d="M ${x} 60 L ${x} 78" class="proc-dose-line"/><text x="${x}" y="4" text-anchor="middle">Fällung</text></g>`,
    );
  }
  if (c.digestion || c.dewatering) {
    let x = start + Math.max(2, flow.length - 2) * gap;
    if (c.digestion)
      extras.push(
        `<g class="procedure-node ${interactive ? "is-interactive" : ""}" data-procedure-node="digestion" tabindex="${interactive ? "0" : "-1"}"><path d="M ${x - 38} 244 L ${x - 30} 194 Q ${x} 160 ${x + 30} 194 L ${x + 38} 244 Z" class="proc-sludge"/><text x="${x}" y="267" text-anchor="middle">Faulung</text></g>`,
      );
    if (c.dewatering)
      extras.push(
        `<g class="procedure-node ${interactive ? "is-interactive" : ""}" data-procedure-node="dewatering" tabindex="${interactive ? "0" : "-1"}"><rect x="${x + 75}" y="194" width="100" height="58" rx="17" class="proc-sludge"/><text x="${x + 125}" y="277" text-anchor="middle">Entwässerung</text></g>`,
      );
  }
...
function bindProcedureCard(root = document) {
  root.querySelectorAll(".procedure-card").forEach((card) => {
    card.querySelector(".procedure-pause")?.addEventListener("click", (e) => {
    ...
    card.querySelectorAll("[data-procedure-node]").forEach((node) => {
      const activate = () => {
        card
          .querySelectorAll("[data-procedure-node]")
        ...
        const label =
          ...
          "Anlagenteil";
        card.querySelector(".procedure-selection").textContent =
          `${label} ausgewählt`;
      };
...
function locationQuery(plant) {
  const latitude = String(plant.address?.latitude || "")
    .replace(",", ".")
    .trim();
  const longitude = String(plant.address?.longitude || "")
    .replace(",", ".")
    .trim();
...
function renderPlantSelector() {
  ...
  select.innerHTML = plants.length
    ? plants
        .map(
          (p) =>
            `<option value="${p.id}" ${p.id === activePlantId ? "selected" : ""}>${esc(p.master.name || "Unbenannte Anlage")}</option>`,
        )
        .join("")
    : `<option value="">Keine Anlage angelegt</option>`;
  select.disabled = !plants.length;
  const plant = activePlant();
  $("#activePlantBadge").textContent = plant
    ? plant.master.name || "Unbenannte Anlage"
...
function updatePrimaryNavigation() {
  $$("[data-primary-view]").forEach((button) => {
    ...
    const calculatorActive =
      target === "calculators" && state.view === "calculators";
    const plantActive =
      target === "plants" &&
      ["plants", "plantForm", "plantDashboard", "limits", "traffic"].includes(
        state.view,
      );
  ...
  $$("[data-global-view]").forEach((button) => {
    ...
    const active =
      (target === "today" && state.view === "dashboard") ||
      state.view === `global-${target}`;
  ...
  cards.innerHTML = list.length
    ? list
        .map(
          (
            item,
          ) => `<article class="calculator-card ${state.selected === item.id ? "active" : ""}" data-id="${item.id}" role="button" tabindex="0">
    ...
    <h3>${item.name}</h3><p>${item.short}</p>
        ...
        .join("")
    : `<div class="no-results"><h3>Keine Treffer</h3><p>Suchbegriff oder Filter ändern.</p></div>`;
  $$(".calculator-card").forEach((card) => {
    card.onclick = (e) => {
      if (!e.target.closest(".favorite")) selectCalculator(card.dataset.id);
    };
    card.onkeydown = (e) => {
      if (
        (e.key === "Enter" || e.key === " ") &&
        ...
        e.preventDefault();
        selectCalculator(card.dataset.id);
      }
    };
  });
  $$(".favorite").forEach(
...
function showCategory(category) {
  ...
  $("#catalogDescription").textContent =
    meta.description || "Verfügbare Rechner";
...
function showAllCalculators() {
  ...
  $("#favoriteFilter").textContent = "★ Favoriten";
  setView("calculators");
  $("#catalogEyebrow").textContent = "Werkzeuge";
  $("#catalogTitle").textContent = "Alle Rechner";
  $("#catalogDescription").textContent =
    "Alle verfügbaren Rechner, gegliedert nach Fachgebiet.";
  workspace.innerHTML = `<div class="empty-state"><h2>Rechner auswählen</h2><p>Wähle ein Werkzeug aus den Karten.</p></div>`;
  setBreadcrumb("Alle Rechner");
  renderCards();
  renderCategoryMenu();
}
function showSearchResults() {
  ...
  $("#catalogDescription").textContent = state.query
    ? `Treffer für „${state.query}“`
    : "Alle verfügbaren Rechner";
...
function showFavorites() {
  ...
  $("#catalogDescription").textContent =
    "Deine lokal auf diesem Gerät gespeicherten Rechner.";
...
function selectCalculator(id) {
  ...
  state.recent = [id, ...state.recent.filter((itemId) => itemId !== id)].slice(
    0,
    6,
  );
...
function upcomingVisits(limit = 4) {
  ...
  return plants
    .flatMap((plant) =>
      (plant.visits || []).map((visit) => ({
        plant,
        visit,
        date: isoLocalToDate(visit.start),
      })),
    )
    .filter(
      (item) =>
        ...
        item.visit.status !== "done" &&
        item.visit.status !== "cancelled",
    )
...
const plantZones = {
  inlet: {
    title: "Zulauf und Vorklärung",
    subtitle: "Frachten, Konzentrationen und hydraulische Belastung",
    icon: "↘",
    calculators: ["load", "concentration", "hrt"],
  },
  biology: {
    title: "Biologische Stufe",
    subtitle: "Belebung, Biomasse und Absetzverhalten",
    icon: "◎",
    calculators: ["loading", "sludge-age", "svi"],
  },
  precipitation: {
    title: "Fällmittelstation",
    subtitle: "Phosphorelimination und Chemikaliendosierung",
    icon: "◆",
    calculators: ["precipitation", "dose", "cost"],
  },
  clarifier: {
    title: "Nachklärung",
    subtitle: "Absetzverhalten und hydraulische Aufenthaltszeit",
    icon: "◉",
    calculators: ["svi", "sludge-age", "hrt"],
  },
  sludge: {
    title: "Schlammbehandlung",
    subtitle: "Feststoffmengen, Lagerung und Reichweiten",
    icon: "≋",
    calculators: ["dw-throughput", "dw-cake", "tank"],
  },
  dewatering: {
    title: "Schlammentwässerung",
    subtitle: "Maschinenleistung, Polymer und Entsorgung",
    icon: "▦",
    calculators: ["dw-polymer", "dw-retention", "dw-disposal", "dw-compare"],
  },
};
function renderPlantAnimation() {
  return `<div class="plant-animation" aria-label="Interaktive schematische Kläranlage aus der Vogelperspektive">
    ...
    <div class="plant-hotspots" aria-label="Anlagenteile">
      ${Object.entries(plantZones)
        .map(
          ([id, zone]) =>
            `<button type="button" class="plant-hotspot zone-${id}" data-plant-zone="${id}" aria-label="${zone.title} öffnen"><span>${zone.title}</span></button>`,
        )
        .join("")}
    </div>
...
function openPlantSheet(zoneId) {
  const zone = plantZones[zoneId];
  if (!zone) return;
  const sheet = $("#plantSheet"),
    backdrop = $("#plantSheetBackdrop");
  $("#plantSheetTitle").textContent = zone.title;
  $("#plantSheetSubtitle").textContent = zone.subtitle;
  $("#plantSheetIcon").textContent = zone.icon;
  $("#plantSheetActions").innerHTML = zone.calculators
    .map((id) => {
      const item = calculators.find((c) => c.id === id);
      return item
        ? `<button type="button" data-sheet-calculator="${item.id}"><span><strong>${item.name}</strong><small>${item.short}</small></span><b>›</b></button>`
        : "";
    })
    .join("");
  sheet.hidden = false;
  ...
  $$("[data-plant-zone]").forEach((button) =>
    button.classList.toggle("selected", button.dataset.plantZone === zoneId),
  );
  $$("[data-sheet-calculator]").forEach(
    (button) =>
      (button.onclick = () => {
        closePlantSheet();
        selectCalculator(button.dataset.sheetCalculator);
      }),
  );
...
function renderDashboard() {
  ...
  const recentList = state.recent
    ...
    .filter(Boolean)
    .slice(0, 4);
  const favoriteList = calculators
  ...
  const capacity = plant?.master?.capacityPE
    ? `${fmtInteger(plant.master.capacityPE)} EW`
    : "Ausbaugröße nicht hinterlegt";
  const plantType =
    plant?.master?.type === "industrial"
      ? "Industrielle Kläranlage"
      : plant?.master?.type === "mixed"
        ? "Kommunale Anlage mit Industrieanteil"
  ...
  $("#dashboard").innerHTML = `
    ...
    <section class="dashboard-section calculator-categories-home">
      ...
      <div class="category-home-grid">${categories
        .map((category) => {
          const meta = categoryMeta[category] || { icon: "∑", description: "" };
          return `<button type="button" data-dashboard-action="${category}"><span>${meta.icon}</span><strong>${category}</strong><small>${categoryCount(category)} Rechner</small></button>`;
        })
        .join("")}</div>
    </section>
    ${renderPlantSheet()}`;
  ...
  if (animationToggle)
    animationToggle.onclick = () => {
      ...
      animationToggle.textContent = paused
        ? "▶ Animation starten"
        : "Ⅱ Animation pausieren";
    };
  $$("[data-plant-zone]").forEach(
    (button) =>
      (button.onclick = () => openPlantSheet(button.dataset.plantZone)),
  );
  $("#plantSheetClose").onclick = closePlantSheet;
  $("#plantSheetCancel").onclick = closePlantSheet;
  $("#plantSheetBackdrop").onclick = closePlantSheet;
}
function bindDashboardActions() {
  ...
  $$("[data-dashboard-calculator]").forEach(
    (button) =>
      (button.onclick = () =>
        selectCalculator(button.dataset.dashboardCalculator)),
  );
}
...
const DOCUMENT_TYPES = [
  ["sds", "Sicherheitsdatenblatt"],
  ["factsheet", "Factsheet"],
  ["product-data", "Produktdatenblatt"],
  ["technical-sheet", "Technisches Merkblatt"],
  ["offer", "Angebot"],
...
function normalizeDocument(d = {}) {
  return {
    ...
    notes: d.notes || "",
    textPreview: String(d.textPreview || d.rawText || "").slice(0, 4000),
    textExtracted: Boolean(d.textExtracted),
    reviewedAt: d.reviewedAt || "",
    reviewer: d.reviewer || "",
    extracted: d.extracted || {},
    source: d.source || "PDF-Import",
    storageState: d.storageState || "stored",
...
function saveDocuments() {
  documentSaveQueue = documentSaveQueue
    .then(() => documentRepository.replaceAll(documents))
    .catch((e) => {
      console.error(e);
      alert("Dokumentmetadaten konnten nicht in IndexedDB gespeichert werden.");
    });
  return true;
}
async function loadDocumentsFromDatabase() {
  documents = (await documentRepository.list()).map(normalizeDocument);
  return documents;
}
...
function docTypeLabel(type) {
  return DOCUMENT_TYPES.find((x) => x[0] === type)?.[1] || "Sonstiges";
}
function docStatusLabel(status) {
  return DOCUMENT_STATUSES.find((x) => x[0] === status)?.[1] || status;
}
function detectGeneralDocumentType(text, name) {
  const h = `${name} ${text}`.toLowerCase();
  if (/sicherheitsdatenblatt|safety data sheet|abschnitt 1.*stoff/i.test(h))
    return "sds";
  if (/auftragsbestätigung|order confirmation/i.test(h))
    return "order-confirmation";
  if (/angebot|quotation|gültig bis/i.test(h)) return "offer";
  if (/lieferschein|delivery note/i.test(h)) return "delivery-note";
  if (/rechnung|invoice/i.test(h)) return "invoice";
  if (/laborbericht|laboratory report/i.test(h)) return "lab-report";
  if (/versuchsbericht|trial report/i.test(h)) return "trial-report";
  if (/factsheet|produktvorteile|ihre vorteile/i.test(h)) return "factsheet";
  if (/produktdatenblatt|product data sheet/i.test(h)) return "product-data";
  return detectDocumentType(text, name);
}
...
async function importDocuments(files) {
  ...
  if (!pdfs.length) return alert("Bitte mindestens eine PDF-Datei auswählen.");
  for (const file of pdfs) {
    const id = makeId();
    const doc = normalizeDocument({
      ...
      size: file.size,
      status: "imported",
      storageState: "storing",
    });
    documents.unshift(doc);
    saveDocuments();
    showDocuments();
    try {
      await documentRepository.saveFile(id, file);
      doc.storageState = "stored";
      await documentRepository.save(doc);
      try {
        const rawText = await extractPdfTextBasic(file);
        doc.type = detectGeneralDocumentType(rawText, file.name);
        doc.textPreview = rawText.slice(0, 4000);
        doc.textExtracted = Boolean(rawText);
        doc.extracted = inferProductFromPdf(rawText, file.name, doc.type);
      ...
      (d) =>
        ...
        (!q ||
          [
            ...
            d.notes,
            ...d.tags,
            ...d.productIds.map((id) => productById(id)?.name || ""),
          ]
            .join(" ")
            .toLowerCase()
            .includes(q)),
    );
    $("#documentLibrary").innerHTML = list.length
      ? `<div class="document-library-list">${list.map((d) => `<article><div class="document-icon">PDF</div><div class="document-main"><div class="document-meta"><span class="status-chip ${d.status === "approved" ? "green" : d.status === "review" ? "amber" : d.status === "archived" ? "gray" : "blue"}">${docStatusLabel(d.status)}</span><span>${docTypeLabel(d.type)}</span><span>${(d.size / 1024 / 1024).toFixed(2)} MB</span><span>${d.storageState === "stored" ? "offline gespeichert" : d.storageState === "error" ? "Speicherfehler" : "wird gespeichert …"}</span></div><h2>${esc(d.fileName)}</h2><p>${[d.documentNumber, d.documentDate, d.customer, productNames(d.productIds)].filter(Boolean).map(esc).join(" · ") || "Noch keine fachliche Zuordnung"}</p></div><div class="document-list-actions"><button class="button secondary compact" data-open-document="${d.id}">Anzeigen</button><button class="button primary compact" data-review-document="${d.id}">${d.status === "imported" ? "Prüfen" : "Bearbeiten"}</button></div></article>`).join("")}</div>`
      : `<div class="empty-panel"><h2>Keine Dokumente gefunden</h2><p>Lade eine PDF hoch. Die Originaldatei wird vollständig in IndexedDB gespeichert und bleibt offline anzeigbar.</p></div>`;
    $$("[data-open-document]").forEach(
      (b) => (b.onclick = () => showDocumentDetail(b.dataset.openDocument)),
    );
    $$("[data-review-document]").forEach(
      (b) => (b.onclick = () => showDocumentReview(b.dataset.reviewDocument)),
    );
  };
  $("#documentPdfImport").onchange = (e) => {
    importDocuments(e.target.files);
    e.target.value = "";
  };
  $("#documentSearch").oninput = render;
  $("#documentStatusFilter").onchange = render;
  $("#documentTypeFilter").onchange = render;
  $$("[data-doc-status-card]").forEach(
    (b) =>
      (b.onclick = () => {
        $("#documentStatusFilter").value = b.dataset.docStatusCard;
        render();
      }),
  );
  $("#documentStorageInfo").onclick = async () => {
    const e = await estimateStorage();
    alert(
      e
        ? `Belegt: ${(e.usage / 1024 / 1024).toFixed(1)} MB\nVerfügbares Kontingent: ${(e.quota / 1024 / 1024).toFixed(1)} MB\n\nPDF-Dateien werden vollständig offline gespeichert.`
        : "Der Browser stellt keine Speicherstatistik bereit. PDF-Dateien werden dennoch lokal in IndexedDB gespeichert.",
    );
  };
  render();
}
...
async function showDocumentDetail(id) {
  ...
  if (!d) return showDocuments();
  state.view = "document-detail";
  setView("document-detail");
  setBreadcrumb(`Dokumente › ${d.fileName}`);
...
async function showDocumentReview(id) {
  ...
  $("#cancelDocumentReview").onclick = () => showDocumentDetail(d.id);
  $("#documentReviewForm").onsubmit = (e) => {
    e.preventDefault();
    const fd = new FormData(e.currentTarget);
    d.type = String(fd.get("type"));
    d.status = String(fd.get("status"));
    for (const k of [
      "documentNumber",
      "documentDate",
      "version",
    ...
    d.tags = String(fd.get("tags") || "")
      .split(",")
      .map((x) => x.trim())
      .filter(Boolean);
    d.productIds = fd.getAll("productIds").map(String);
    const newName = String(fd.get("newProductName") || "").trim();
    if (newName) {
      let p = products.find(
      ...
      if (!p) {
        p = normalizeProduct();
        p.name = newName;
        p.materialNumber = String(fd.get("newMaterialNumber") || "").trim();
        p.category =
          String(fd.get("newProductCategory") || "").trim() || "Sonstiges";
        products.push(p);
      }
      if (!d.productIds.includes(p.id)) d.productIds.push(p.id);
      if (!p.documents.some((x) => x.id === d.id))
        p.documents.push({
          id: d.id,
          fileName: d.fileName,
          type: d.type,
          ...
          source: "Dokumentenzentrale",
          reviewStatus: d.status === "approved" ? "confirmed" : "review",
          textExtracted: d.textExtracted,
        });
    ...
    for (const pid of d.productIds) {
      ...
      if (p && !p.documents.some((x) => x.id === d.id)) {
        p.documents.push({
          id: d.id,
          fileName: d.fileName,
          type: d.type,
    ...
    d.reviewer = employeeProfile?.name || "lokaler Benutzer";
    d.updatedAt = new Date().toISOString();
    saveProducts();
    saveDocuments();
    showDocumentDetail(d.id);
  };
}
...
function showGlobalPage(page) {
  ...
  page = valid.has(page) ? page : "today";
  localStorage.setItem(STORAGE_GLOBAL_PAGE, page);
  if (page === "today") return showHome();
  setView(`global-${page}`);
  const titles = {
    appointments: "Termine",
    "tasks-global": "Aufgaben",
    documents: "Dokumente",
    products: "Produkte",
    projects: "Optimierungsprojekte",
    reports: "Berichte",
    backup: "Backup",
  ...
  if (page === "appointments") {
    ...
    const tasks = plants
      ...
      .sort((a, b) =>
        (a.action.dueDate || "9999").localeCompare(b.action.dueDate || "9999"),
      );
    ...
    $("#globalExportBackup").onclick = () =>
      downloadJson(
        `abwasser-rechner-sicherung-${new Date().toISOString().slice(0, 10)}.json`,
        {
          schema: "abwasser-rechner-backup-v1",
          version: VERSION,
    ...
    $("#globalImportBackup").onchange = async (e) => {
      const file = e.target.files?.[0];
      if (!file) return;
      try {
        const data = JSON.parse(await file.text());
        if (!Array.isArray(data.plants) || !data.employeeProfile)
          throw new Error("Keine gültige Gesamtsicherung");
        if (
          !confirm(
            "Vorhandene Profil- und Anlagendaten durch diese Sicherung ersetzen?",
          )
        )
          return;
        ...
        products = Array.isArray(data.products)
          ...
          : products;
        saveProducts();
        employeeProfile = normalizeEmployeeProfile(data.employeeProfile);
        activePlantId =
          data.activePlantId && plants.some((x) => x.id === data.activePlantId)
            ? data.activePlantId
            : plants[0]?.id || "";
        if (savePlants() && saveEmployeeProfile()) {
          renderPlantSelector();
          updateProfileButton();
          showGlobalPage("backup");
        }
      } catch (err) {
    ...
    const visitCount = plants.reduce((n, p) => n + (p.visits || []).length, 0),
      actionCount = plants.reduce((n, p) => n + (p.actions || []).length, 0);
    ...
    Promise.all([
      documentRepository.diagnostics(),
      navigator.storage?.estimate?.() || null,
      recentAudit(5),
    ])
      .then(([diag, storage, events]) => {
        const host = $("#v011Diagnostics");
        if (!host) return;
        ...
        if (events.length)
          appView.insertAdjacentHTML(
            "beforeend",
            `<article class="release-notes"><h2>Letzte Systemereignisse</h2><ul>${events.map((e) => `<li>${esc(new Date(e.createdAt).toLocaleString("de-DE"))}: ${esc(e.action)}</li>`).join("")}</ul></article>`,
          );
      })
      .catch(console.error);
    ...
    appView.innerHTML = renderGlobalPlaceholder(
      "📈",
      "Optimierungsprojekte",
      "Anlagenübergreifende Pipeline für Analysen, Versuche, Angebote und Aufträge.",
      "Die Projektlogik wird nach der Dokumenten- und Produktbasis umgesetzt.",
    );
  else if (page === "reports")
    appView.innerHTML = renderGlobalPlaceholder(
      "📊",
      "Berichte",
      "Besuchsberichte, Jahresübersichten und technische Auswertungen.",
      "Berichte werden schrittweise aus Anlagen-, Besuchs- und Projektdaten erzeugt.",
    );
  else if (page === "settings")
    appView.innerHTML = renderGlobalPlaceholder(
      "⚙",
      "Einstellungen",
      "Appweite Einstellungen für Darstellung, Backup-Erinnerungen und zukünftige Benutzeroptionen.",
      "Die lokale Benutzer- und PIN-Sperre ist als späterer Foundation-Baustein vorgesehen.",
    );
  $$("[data-global-open-plant]").forEach(
    (b) =>
      (b.onclick = () => {
        activePlantId = b.dataset.globalOpenPlant;
        savePlants();
        showPlantDashboard(b.dataset.globalPlantPage || "overview");
      }),
  );
}

function showProducts() {
  state.view = "global-products";
  localStorage.setItem(STORAGE_GLOBAL_PAGE, "products");
  setView("global-products");
  setBreadcrumb("Produkte");
  updatePrimaryNavigation();
  ...
  const render = (query = "") => {
    ...
    $("#productLibrary").innerHTML = list.length
      ? `<div class="product-grid">${list
          .map(
            (p) =>
              `<article class="product-card"><div class="product-card-head"><span class="status-chip blue">${esc(p.category)}</span><span>${p.documents.length} Dokument${p.documents.length === 1 ? "" : "e"}</span></div><h2>${esc(p.name || "Unbenanntes Produkt")}</h2><p>${esc(p.shortDescription || "Noch keine Kurzbeschreibung hinterlegt.")}</p><div class="product-tags">${p.problems
                .slice(0, 4)
                .map((x) => `<span>${esc(x)}</span>`)
                .join(
                  "",
                )}</div><dl><div><dt>Materialnummer</dt><dd>${esc(p.materialNumber || "–")}</dd></div><div><dt>Prüfstatus</dt><dd>${p.reviewStatus === "confirmed" ? "Geprüft" : p.reviewStatus === "seeded" ? "Vorbelegt" : "Entwurf"}</dd></div></dl><button class="button secondary" type="button" data-open-product="${p.id}">Produkt öffnen</button></article>`,
          )
          .join("")}</div>`
      : `<div class="empty-panel"><h2>Keine Produkte gefunden</h2><p>Importiere ein PDF oder lege ein Produkt manuell an.</p></div>`;
    $$("[data-open-product]").forEach(
      (b) => (b.onclick = () => showProductDetail(b.dataset.openProduct)),
    );
  };
  render();
  renderProductImportQueueStatus();
  $("#productSearch").oninput = (e) => render(e.target.value);
  $("#productPdfImport").onchange = handleProductPdfImport;
  $("#newProductManual").onclick = () => showProductEditor();
}
async function handleProductPdfImport(event) {
  const files = event.target.files;
  if (!files?.length) return;
  enqueueProductPdfs(files);
  event.target.value = "";
}

async function showProductImportReview(
  file,
  type,
  inferred,
  rawText,
  ...
  state.view = "product-import";
  setView("product-import");
  setBreadcrumb("Produkte › PDF prüfen");
  ...
  const quality =
    queueItem?.quality || productImportQuality(type, inferred, rawText);
  ...
  $("#cancelProductImport").onclick = () =>
    queueItem ? showProducts() : showProducts();
  $("#productImportReview").onsubmit = async (e) => {
    e.preventDefault();
    const fd = new FormData(e.currentTarget);
    let product = productById(String(fd.get("existingProduct") || ""));
    if (!product) {
      product = normalizeProduct();
      products.push(product);
    }
    product.name = String(fd.get("name") || "").trim();
    product.materialNumber = String(fd.get("materialNumber") || "").trim();
    product.category = String(fd.get("category") || "").trim() || "Sonstiges";
    product.shortDescription = String(fd.get("shortDescription") || "").trim();
    product.applications = unique([
      ...product.applications,
      ...splitKnowledge(fd.get("applications")),
    ]);
    product.problems = unique([
      ...product.problems,
      ...splitKnowledge(fd.get("problems")),
    ]);
    product.benefits = unique([
      ...product.benefits,
      ...splitKnowledge(fd.get("benefits")),
    ]);
    product.technical.state =
      String(fd.get("state") || "").trim() || product.technical.state;
    product.technical.ph =
      String(fd.get("ph") || "").trim() || product.technical.ph;
    product.technical.density =
      String(fd.get("density") || "").trim() || product.technical.density;
    product.safety.signalWord =
      String(fd.get("signalWord") || "").trim() || product.safety.signalWord;
    product.safety.unNumber =
      String(fd.get("unNumber") || "").trim() || product.safety.unNumber;
    product.safety.hazardStatements = unique([
      ...product.safety.hazardStatements,
      ...splitKnowledge(fd.get("hazardStatements")),
    ]);
    const docId = queueItem?.tempFileId || makeId();
    if (!queueItem?.tempFileId) await storeProductFile(docId, file);
    product.documents.push({
      id: docId,
      fileName: file.name,
      type,
      documentDate: String(fd.get("documentDate") || "").trim(),
      size: file.size,
      mimeType: file.type || "application/pdf",
    ...
    if (queueItem) {
      ...
      queueItem.productId = product.id;
    }
    if (saveProducts()) {
      if (productImportQueue.length > 1) showProducts();
      else showProductDetail(product.id);
    }
  };
}
function showProductDetail(id) {
  const p = productById(id);
  if (!p) return showProducts();
  state.view = "product-detail";
  setView("product-detail");
  setBreadcrumb(`Produkte › ${p.name}`);
  ...
  $("#editProduct").onclick = () => showProductEditor(p.id);
  $("#addProductPdf").onchange = handleProductPdfImport;
  $$("[data-open-product-document]").forEach(
    (b) =>
      (b.onclick = async () => {
        const blob =
          (await documentRepository.getFile(b.dataset.openProductDocument)) ||
          (await getProductFile(b.dataset.openProductDocument));
        if (!blob) return alert("Die lokale PDF-Datei wurde nicht gefunden.");
        const url = URL.createObjectURL(blob);
        window.open(url, "_blank", "noopener");
        setTimeout(() => URL.revokeObjectURL(url), 60000);
      }),
  );
  $$("[data-delete-product-document]").forEach(
    (b) =>
      (b.onclick = async () => {
        if (!confirm("Dokument wirklich aus der Produktakte entfernen?"))
          return;
        await deleteProductFile(b.dataset.deleteProductDocument);
        p.documents = p.documents.filter(
          (d) => d.id !== b.dataset.deleteProductDocument,
        );
        saveProducts();
...
function showProductEditor(id = "") {
  let p = id ? productById(id) : normalizeProduct();
  if (!p) return showProducts();
  state.view = "product-editor";
  setView("product-editor");
  setBreadcrumb(
    id ? `Produkte › ${p.name} › Bearbeiten` : "Produkte › Neues Produkt",
  );
  ...
  $("#cancelProductEditor").onclick = () =>
    id ? showProductDetail(id) : showProducts();
  $("#productEditor").onsubmit = (e) => {
    e.preventDefault();
    const fd = new FormData(e.currentTarget);
    p.name = String(fd.get("name") || "").trim();
    p.materialNumber = String(fd.get("materialNumber") || "").trim();
    p.category = String(fd.get("category") || "").trim() || "Sonstiges";
    p.shortDescription = String(fd.get("shortDescription") || "").trim();
    p.applications = splitKnowledge(fd.get("applications"));
    p.problems = splitKnowledge(fd.get("problems"));
    p.benefits = splitKnowledge(fd.get("benefits"));
    for (const k of [
      "state",
      "ph",
      "density",
      ...
      p.technical[k] = String(fd.get(k) || "").trim();
    p.safety.signalWord = String(fd.get("signalWord") || "").trim();
    p.safety.unNumber = String(fd.get("unNumber") || "").trim();
    p.safety.hazardStatements = splitKnowledge(fd.get("hazardStatements"));
    p.updatedAt = new Date().toISOString();
    p.reviewStatus = "confirmed";
    if (!id) products.push(p);
    if (saveProducts()) showProductDetail(p.id);
  };
}
...
function showApplication(view) {
  ...
  setView("plants");
  setBreadcrumb("Anlagenübersicht");
  renderPlants();
}
  ...
  <div class="plant-grid">${
    plants.length
      ? plants
          .map(
            (
              p,
            ) => `<article class="plant-card ${p.id === activePlantId ? "active" : ""}">
    ...
    <div class="card-actions">
      ...
      <button type="button" class="danger-link" data-delete-plant="${p.id}">Löschen</button>
          ...
          .join("")
      : `<div class="empty-panel"><h2>Noch keine Anlage angelegt</h2><p>Lege die erste Anlagenakte mit Stammdaten, Betreiber und Ansprechpartner an.</p></div>`
  }</div>`;
  $("#createPlantTop").onclick = () => showPlantForm();
  $$("[data-open-plant]").forEach(
    (b) =>
      (b.onclick = () => {
        activePlantId = b.dataset.openPlant;
        savePlants();
        showPlantDashboard();
      }),
  );
  $$("[data-edit-plant]").forEach(
    (b) => (b.onclick = () => showPlantForm(b.dataset.editPlant)),
  );
  $$("[data-delete-plant]").forEach(
    (b) =>
      (b.onclick = () => {
        const p = plants.find((x) => x.id === b.dataset.deletePlant);
        if (
          confirm(
            `Anlage „${p?.master.name || "Unbenannte Anlage"}“ wirklich löschen?`,
          )
        ) {
          plants = plants.filter((x) => x.id !== b.dataset.deletePlant);
          if (activePlantId === b.dataset.deletePlant)
            activePlantId = plants[0]?.id || "";
          savePlants();
          renderPlants();
        }
...
function field(name, label, value = "", type = "text", placeholder = "") {
  ...
  if (type === "number") {
    if (name === "address.latitude")
      numericAttributes = ' step="any" min="-90" max="90" inputmode="decimal"';
    else if (name === "address.longitude")
      numericAttributes =
        ' step="any" min="-180" max="180" inputmode="decimal"';
    else numericAttributes = ' step="any" inputmode="decimal"';
  }
  const effectiveType =
    name === "address.latitude" || name === "address.longitude" ? "text" : type;
...
function dosingDefaults(value = {}) {
  return {
    ...
    capacityLh: "",
    maxPressureBar: "",
    controlMode: "flow-proportional",
    standbyPump: false,
    automaticChangeover: false,
    calibrationDevice: false,
    flushConnection: false,
    pressureHoldingValve: false,
    overflowValve: false,
    pulsationDamper: false,
    productName: "",
    activeIngredient: "",
...
function dewateringProcessText(value) {
  return (
    {
      "screw-press": "Schneckenpresse",
      "belt-press": "Siebbandpresse",
      "filter-press": "Kammerfilterpresse",
      centrifuge: "Zentrifuge",
      mobile: "Mobile Entwässerung",
      dryingBed: "Trockenbeet",
      other: "Sonstiges",
    }[value] || "Nicht festgelegt"
  );
}
function dosingPurposeText(value) {
  return (
    {
      polymer: "Polymer",
      precipitant: "Fällmittel",
      carbon: "Kohlenstoffquelle",
      neutralization: "Neutralisationsmittel",
...
function showPlantForm(id = null) {
  setView("plantForm");
  setBreadcrumb(id ? "Anlage bearbeiten" : "Neue Anlage");
  ...
  p.master.processStages = Array.isArray(p.master.processStages)
    ? p.master.processStages
    : [];
  p.master.processOther = p.master.processOther || p.master.process || "";
  p.sludgeDewatering = dewateringDefaults(p.sludgeDewatering || {});
  p.dosingSystems = Array.isArray(p.dosingSystems)
    ? p.dosingSystems.map(dosingDefaults)
    : [];
  p.tankSystems = Array.isArray(p.tankSystems)
    ? p.tankSystems.map(tankDefaults)
    : [];
  ...
  appView.innerHTML = `<form id="plantForm" class="record-form">
    ...
    <section class="form-section"><h2>Stammdaten und Verfahrenstechnik</h2><div class="form-grid">
      ...
      ${selectField("master.type", "Anlagentyp", p.master.type, [
        ["municipal", "Kommunal"],
        ["industrial", "Industriell"],
        ["mixed", "Kommunal mit Industrieanteil"],
      ])}
    ...
    <section class="form-section technical-form-section"><div class="section-heading"><div><p class="eyebrow">Schlammbehandlung</p><h2>Schlammentwässerung</h2><p class="form-note">Maschine, Leistung und zugehörige Peripherie in der Anlagenakte dokumentieren.</p></div></div>
      ...
      <div class="form-grid" id="dewateringFields">
        ${selectField(
          "sludgeDewatering.status",
          "Betriebsstatus",
          p.sludgeDewatering.status,
          [
            ["active", "In Betrieb"],
        ...
        ${selectField(
          "sludgeDewatering.process",
          "Verfahren",
          p.sludgeDewatering.process,
          [
            ["screw-press", "Schneckenpresse"],
            ["belt-press", "Siebbandpresse"],
            ["filter-press", "Kammerfilterpresse"],
        ...
        ${selectField(
          "sludgeDewatering.operationMode",
          "Betriebsweise",
          p.sludgeDewatering.operationMode,
          [
  ...
  if (!plantForm) {
    ...
    alert(
      "Das Anlagenformular konnte nicht geladen werden. Bitte die App vollständig neu starten.",
    );
    return;
  }
  ...
  let dosingSystems = structuredClone(p.dosingSystems || []).map(
    dosingDefaults,
  );
  ...
  const syncDosingSystemsFromForm = () => {
    dosingSystems = dosingSystems.map((current, i) => {
      ...
      for (const key of Object.keys(out)) {
        ...
        out[key] =
          typeof out[key] === "boolean" ? Boolean(input.checked) : input.value;
      }
      return out;
    });
  };
  const renderDosingSystems = () => {
    dosingEditor.innerHTML = dosingSystems.length
      ? dosingSystems
          .map(
            (
              d,
              i,
            ) => `<article class="dosing-editor-card"><div class="contact-editor-head"><strong>${esc(d.name || `Dosieranlage ${i + 1}`)}</strong><button type="button" data-remove-dosing="${i}">Entfernen</button></div>
      <div class="dosing-block"><h3>Zuordnung und Status</h3><div class="form-grid">${field(`dosing.${i}.name`, "Bezeichnung", d.name)}${selectField(
        `dosing.${i}.purpose`,
        "Verwendungszweck",
        d.purpose,
        [
      ...
      )}${selectField(`dosing.${i}.status`, "Betriebsstatus", d.status, [
        ["active", "In Betrieb"],
        ["inactive", "Außer Betrieb"],
        ["reserve", "Reserve"],
        ["planned", "Geplant"],
      ])}${field(`dosing.${i}.location`, "Standort / Einbauort", d.location)}</div></div>
      <div class="dosing-block"><h3>Tankanlage</h3><div class="form-grid">${selectField(
        `dosing.${i}.tankType`,
        "Tankart",
        d.tankType,
        [
          ...
          ["day-tank", "Tagestank"],
          ["ibc", "IBC"],
          ["double-wall", "Doppelwandtank"],
          ["other", "Sonstiges"],
        ],
      )}${field(`dosing.${i}.tankVolume`, "Volumen [l]", d.tankVolume, "number")}${field(`dosing.${i}.tankManufacturer`, "Hersteller", d.tankManufacturer)}${field(`dosing.${i}.tankModel`, "Typ", d.tankModel)}${field(`dosing.${i}.tankYear`, "Baujahr", d.tankYear, "number")}${field(`dosing.${i}.tankMaterial`, "Material", d.tankMaterial)}<div class="span-2 check-grid">${checkboxField(`dosing.${i}.doubleWalled`, "Doppelwandig", d.doubleWalled)}${checkboxField(`dosing.${i}.bundPresent`, "Auffangwanne", d.bundPresent)}${checkboxField(`dosing.${i}.levelMonitoring`, "Füllstandsüberwachung", d.levelMonitoring)}${checkboxField(`dosing.${i}.leakageMonitoring`, "Leckageüberwachung", d.leakageMonitoring)}</div>${field(`dosing.${i}.lastInspection`, "Letzte Prüfung", d.lastInspection, "date")}${field(`dosing.${i}.nextInspection`, "Nächste Prüfung", d.nextInspection, "date")}</div></div>
      <div class="dosing-block"><h3>Dosierstation</h3><div class="form-grid">${field(`dosing.${i}.stationManufacturer`, "Hersteller", d.stationManufacturer)}${field(`dosing.${i}.stationModel`, "Typ", d.stationModel)}${field(`dosing.${i}.stationYear`, "Baujahr", d.stationYear, "number")}${selectField(
        `dosing.${i}.pumpType`,
        "Pumpenart",
        d.pumpType,
        [
          ["diaphragm", "Membrandosierpumpe"],
          ["hose", "Schlauchpumpe"],
          ["progressive-cavity", "Exzenterschneckenpumpe"],
          ["piston", "Kolbenpumpe"],
          ["other", "Sonstige"],
        ],
      )}${field(`dosing.${i}.pumpCount`, "Anzahl Dosierpumpen", d.pumpCount, "number")}${field(`dosing.${i}.capacityLh`, "Förderleistung [l/h]", d.capacityLh, "number")}${field(`dosing.${i}.maxPressureBar`, "Maximaldruck [bar]", d.maxPressureBar, "number")}${selectField(
        `dosing.${i}.controlMode`,
        "Betriebsweise",
        d.controlMode,
        [
          ["constant", "Konstant"],
          ["flow-proportional", "Durchflussproportional"],
          ["measured-value", "Messwertgeführt"],
          ["manual", "Manuell"],
        ],
  ...
  renderDosingSystems();
  $("#addDosingSystem").onclick = () => {
    syncDosingSystemsFromForm();
    dosingSystems.push(
      dosingDefaults({ name: `Dosieranlage ${dosingSystems.length + 1}` }),
    );
    renderDosingSystems();
  };
  const dewateringEnabled = plantForm.elements.namedItem(
      "sludgeDewatering.enabled",
    ),
    dewateringFields = $("#dewateringFields");
  const syncDewateringVisibility = () => {
    dewateringFields.classList.toggle(
      "disabled-section",
      !dewateringEnabled.checked,
    );
    dewateringFields
      .querySelectorAll("input,select,textarea")
      .forEach((el) => (el.disabled = !dewateringEnabled.checked));
  };
  dewateringEnabled.onchange = syncDewateringVisibility;
  syncDewateringVisibility();
  ...
  const syncContactsFromForm = () => {
    contacts = contacts.map((current, i) => ({
      ...
      department:
        plantForm.elements.namedItem(`contact.${i}.department`)?.value || "",
      email: plantForm.elements.namedItem(`contact.${i}.email`)?.value || "",
      preferred:
        plantForm.elements.namedItem(`contact.${i}.preferred`)?.value ||
        "email",
      notes: plantForm.elements.namedItem(`contact.${i}.notes`)?.value || "",
      phone: [
        plantForm.elements.namedItem(`contact.${i}.phoneParts.code`)?.value ||
          "",
        plantForm.elements.namedItem(`contact.${i}.phoneParts.number`)?.value ||
          "",
      ]
        .filter(Boolean)
        .join(" ")
        .trim(),
      mobile: [
        plantForm.elements.namedItem(`contact.${i}.mobileParts.code`)?.value ||
          "",
        plantForm.elements.namedItem(`contact.${i}.mobileParts.number`)
          ?.value || "",
      ]
        .filter(Boolean)
        .join(" ")
  ...
  const renderContacts = () => {
    editor.innerHTML = contacts.length
      ? contacts
          .map(
            (c, i) => `<article class="contact-editor-card">
      ...
      <div class="form-grid">
        ...
        ${field(`contact.${i}.notes`, "Bemerkung", c.notes || "")}
          ...
          .join("")
      : `<p class="empty-inline">Noch kein Ansprechpartner hinterlegt.</p>`;
    $$("[data-remove-contact]").forEach(
      (b) =>
        (b.onclick = () => {
          syncContactsFromForm();
          contacts.splice(Number(b.dataset.removeContact), 1);
          renderContacts();
        }),
    );
  };
  ...
  const applyReverseAddress = (data) => {
    ...
    const road =
      a.road || a.pedestrian || a.residential || a.path || a.cycleway || "";
    ...
    if (nameInput && !nameInput.value.trim() && city)
      nameInput.value = `Kläranlage ${city}`;
  };
  locationButton.onclick = () => {
    if (!window.isSecureContext) {
      setLocationStatus(
        "Die Standortermittlung ist nur über HTTPS oder localhost verfügbar. Bitte die bereitgestellte Web-App öffnen, nicht die HTML-Datei direkt.",
        "error",
      );
      return;
    }
    if (!navigator.geolocation) {
      setLocationStatus(
        "Dieses Gerät oder dieser Browser unterstützt keine Standortermittlung.",
        "error",
      );
      return;
    }
    ...
    setLocationStatus(
      "GPS-Signal wird gesucht. Dies kann im Gebäude etwas länger dauern.",
      "loading",
    );
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        if (!navigator.onLine) {
          setLocationStatus(
            `Koordinaten gespeichert (Genauigkeit ± ${Math.round(accuracy)} m). Keine Internetverbindung – die Adresse kann später manuell ergänzt werden.`,
            "warning",
          );
        ...
        setLocationStatus(
          `Koordinaten erfasst (Genauigkeit ± ${Math.round(accuracy)} m). Adresse wird ermittelt …`,
          "loading",
        );
        try {
          ...
          setLocationStatus(
            "Standort und Adresse wurden übernommen. Bitte die Angaben vor dem Speichern prüfen.",
            "success",
          );
        } catch (error) {
          console.warn("Rückwärts-Geokodierung fehlgeschlagen", error);
          setLocationStatus(
            "Die Koordinaten wurden gespeichert, aber die Adresse konnte nicht automatisch ermittelt werden. Sie kann manuell ergänzt werden.",
            "warning",
          );
        } finally {
          locationButton.disabled = false;
      ...
      (error) => {
        const messages = {
          1: "Standortfreigabe wurde verweigert. Bitte die Berechtigung im Browser aktivieren oder die Daten manuell eintragen.",
          2: "Der Standort konnte nicht bestimmt werden. Bitte nach draußen gehen oder GPS aktivieren.",
          3: "Die Standortermittlung hat zu lange gedauert. Bitte erneut versuchen.",
        };
        setLocationStatus(
  ...
  ["address.latitude", "address.longitude"].forEach((name) =>
    formInput(name)?.addEventListener("input", renderLocationPreview),
  );
  renderLocationPreview();
  $("#cancelPlant").onclick = () =>
    existing ? showPlantDashboard() : showApplication("plants");
  plantForm.addEventListener(
    "invalid",
    (event) => {
      event.preventDefault();
      const label =
        event.target.closest("label")?.childNodes?.[0]?.textContent?.trim() ||
        "Eingabefeld";
      alert(
        `Bitte das Feld „${label}“ prüfen. Dezimalwerte können mit Punkt oder Komma eingegeben werden.`,
      );
      event.target.focus();
    },
    true,
  ...
  plantForm.onsubmit = (e) => {
    ...
    for (const [key, value] of fd.entries()) {
      if (
        ...
        key.startsWith("dosing.") ||
        key.startsWith("sludgeDewatering.") ||
        key.startsWith("tank.")
      )
        continue;
    ...
    result.master.process =
      result.master.processOther || processLabel(result.master.mainProcess);
    const latitudeRaw = String(formInput("address.latitude")?.value || "")
      .trim()
      .replace(",", ".");
    const longitudeRaw = String(formInput("address.longitude")?.value || "")
      .trim()
      .replace(",", ".");
    ...
    if (
      ...
      return alert("Der Breitengrad muss zwischen -90 und +90 liegen.");
    if (
      longitude !== null &&
      (!Number.isFinite(longitude) || longitude < -180 || longitude > 180)
    )
      return alert("Der Längengrad muss zwischen -180 und +180 liegen.");
    ...
    result.address.accuracy =
      p.address.accuracy ||
      String(result.address.accuracy || "").replace(/[^0-9.,]/g, "");
    result.address.capturedAt =
      p.address.capturedAt || result.address.capturedAt || "";
    result.address.geocodedAt =
      p.address.geocodedAt || result.address.geocodedAt || "";
    const dewatering = dewateringDefaults(
      result.sludgeDewatering || p.sludgeDewatering || {},
    );
    ...
    for (const key of Object.keys(dewatering)) {
      ...
      if (typeof dewatering[key] === "boolean")
        dewatering[key] = fd.has(fieldName);
    ...
    result.tankSystems = Array.isArray(result.tankSystems)
      ? result.tankSystems.map(tankDefaults)
      : [];
    ...
    if (existing) plants = plants.map((x) => (x.id === result.id ? result : x));
    else plants.push(result);
    activePlantId = result.id;
...
function getEvaluation(limit, value) {
  ...
  if (!Number.isFinite(n) || value === "")
    return {
      ...
      reason: "Messwert fehlt",
    };
  if (limit.direction === "max") {
    if (n <= Number(limit.target))
      return {
        level: "green",
        label: "Im Zielbereich",
        reason: `≤ ${fmt(limit.target)} ${limit.unit}`,
      };
    if (n <= Number(limit.warning))
      return {
        level: "yellow",
        label: "Beobachten",
        reason: `über Ziel, bis ${fmt(limit.warning)} ${limit.unit}`,
      };
    if (limit.legal !== null && limit.legal !== "" && n > Number(limit.legal))
      return {
        level: "red",
        label: "Grenzwert überschritten",
        reason: `> ${fmt(limit.legal)} ${limit.unit}`,
      };
    return {
      level: "red",
      label: "Handlungsbedarf",
      reason: `> ${fmt(limit.warning)} ${limit.unit}`,
    };
  }
  if (limit.direction === "min") {
    ...
    if (n >= Number(limit.warning))
      return {
        level: "yellow",
        label: "Beobachten",
        reason: `unter Ziel, mindestens ${fmt(limit.warning)} ${limit.unit}`,
      };
    return {
      level: "red",
      label: "Handlungsbedarf",
  ...
  if (n >= Number(limit.greenMin) && n <= Number(limit.greenMax))
    return {
      level: "green",
      label: "Im Zielbereich",
      reason: `${fmt(limit.greenMin)}–${fmt(limit.greenMax)} ${limit.unit}`,
    };
  if (n >= Number(limit.warningMin) && n <= Number(limit.warningMax))
    return {
      level: "yellow",
      label: "Beobachten",
      reason: `außer Ziel, innerhalb Warnbereich`,
    };
  return {
    level: "red",
    label: "Handlungsbedarf",
    reason: `außer ${fmt(limit.warningMin)}–${fmt(limit.warningMax)} ${limit.unit}`,
  };
}
function evaluations(plant) {
  return (plant.limits || defaultLimits).map((limit) => ({
    ...limit,
    value: plant.parameters[limit.key],
    evaluation: getEvaluation(limit, plant.parameters[limit.key]),
  }));
}
function renderTrafficSummary(plant) {
  const evals = evaluations(plant);
  const tally = { green: 0, yellow: 0, red: 0, gray: 0 };
  evals.forEach((e) => tally[e.evaluation.level]++);
...
function showVisitForm(visitId = null) {
  ...
  const visit = existing
    ...
    : {
        ...
        purpose: "",
        contact: plant.contacts?.[0]?.name || "",
        status: "planned",
        notes: "",
      };
  setView("plantDashboard");
  setBreadcrumb(
    `Anlagen › ${plant.master.name || "Unbenannte Anlage"} › Besuchstermin`,
  );
  appView.innerHTML = `<form id="visitForm" class="record-form">
    ...
    <section class="form-section"><div class="form-grid">
      ...
      ${selectField("status", "Status", visit.status, [
        ["planned", "Geplant"],
        ["done", "Erledigt"],
        ["cancelled", "Abgesagt"],
      ])}
      ...
      ${selectField("visitType", "Besuchsart", visit.visitType, [
        ...
        ["product-trial", "Produktversuch"],
        ["inventory", "Bestandsaufnahme / Rundgang"],
      ...
      ${selectField("processArea", "Prozessbereich", visit.processArea, [
        ["", "Bitte auswählen"],
        ["inlet", "Zulauf / mechanische Stufe"],
        ["biology", "Biologische Stufe"],
        ["secondary-clarifier", "Nachklärung"],
        ["phosphorus", "Phosphatfällung"],
        ["sludge-treatment", "Schlammbehandlung"],
        ["dewatering", "Schlammentwässerung"],
        ["digester", "Faulturm"],
        ["filtrate", "Zentrat / Filtrat"],
        ["channel", "Kanal / Pumpwerk"],
        ["odor", "Abluft / Geruch"],
  ...
  $("#visitForm").onsubmit = (e) => {
    e.preventDefault();
    const fd = new FormData(e.currentTarget);
    const saved = normalizeVisit(
      existing ? { ...existing, id: visit.id } : { id: visit.id },
    );
    for (const key of [
      "title",
      "status",
      ...
      saved[key] = fd.get(key) || "";
    const start = isoLocalToDate(saved.start),
      end = isoLocalToDate(saved.end);
    if (!start || !end || end <= start)
      return alert("Das Terminende muss nach dem Beginn liegen.");
    plant.visits = plant.visits || [];
    plant.visits = existing
      ? plant.visits.map((v) => (v.id === saved.id ? saved : v))
      : [...plant.visits, saved];
  ...
  return visits
    .map(
      ...
      <div class="visit-main"><div class="visit-title-row"><h3>${esc(v.title || "Besuchstermin")}</h3><span class="status-chip ${visitStatusClass(v.status)}">${visitStatusLabel(v.status)}</span></div>
        ${(() => {
          const done = VISIT_CHECKLIST.filter(
            ([key]) => v.checklist?.[key],
          ).length;
          return `<div class="visit-progress"><span style="width:${Math.round((done / VISIT_CHECKLIST.length) * 100)}%"></span></div><small>${done} von ${VISIT_CHECKLIST.length} Besuchspunkten erledigt</small>`;
        })()}
...
function renderVisits(plant) {
  ...
  const upcoming = visits
    .filter(
      (v) =>
        (isoLocalToDate(v.start)?.getTime() || 0) >= now && v.status !== "done",
    )
...
function showVisitMode(visitId = null) {
  ...
  const now = new Date(),
    localValue = (d) =>
      `${d.getFullYear()}-${pad2(d.getMonth() + 1)}-${pad2(d.getDate())}T${pad2(d.getHours())}:${pad2(d.getMinutes())}`;
  let visit = visitId
    ? (plant.visits || []).find((v) => v.id === visitId)
    : null;
  if (!visit) {
    visit = normalizeVisit({
      title: `Besuch ${plant.master.name || "Kläranlage"}`,
      start: localValue(now),
      end: localValue(new Date(now.getTime() + 60 * 60 * 1000)),
      contact: plant.contacts?.[0]?.name || "",
      status: "planned",
      modeStatus: "active",
      startedAt: new Date().toISOString(),
    });
    plant.visits = [...(plant.visits || []), visit];
    plant.updatedAt = new Date().toISOString();
    if (!savePlants()) return;
  } else {
    visit = normalizeVisit(visit);
    if (visit.modeStatus === "not-started") {
      visit.modeStatus = "active";
      visit.startedAt = visit.startedAt || new Date().toISOString();
    }
    plant.visits = plant.visits.map((v) => (v.id === visit.id ? visit : v));
    savePlants();
  }
  const persist = () => {
    plant.visits = plant.visits.map((v) => (v.id === visit.id ? visit : v));
  ...
  const render = () => {
    ...
    setView("plantDashboard");
    setBreadcrumb(
      `Anlagen › ${plant.master.name || "Unbenannte Anlage"} › Besuch`,
    );
    ...
    $("#finishVisit").onclick = () => {
      visit.modeStatus =
        visit.modeStatus === "completed" ? "active" : "completed";
      visit.status = visit.modeStatus === "completed" ? "done" : "planned";
      visit.completedAt =
        visit.modeStatus === "completed" ? new Date().toISOString() : "";
      if (persist()) render();
    };
    $$("[data-check]").forEach(
      (el) =>
        (el.onchange = () => {
          visit.checklist[el.dataset.check] = el.checked;
          persist();
          render();
        }),
    );
    $("#saveMeasurements").onclick = () => {
      for (const key of [
        ...
        const el = appView.querySelector(`[name="vm.${key}"]`);
        visit.measurements[key] = el?.value || "";
      }
      visit.checklist.measurements = true;
      if (persist()) render();
    };
    $("#saveVisitReport").onclick = () => {
      for (const key of [
        "initialSituation",
        "workPerformed",
        "chemistryChanges",
        ...
        visit[key] =
          appView.querySelector(`[name="vr.${key}"]`)?.value.trim() || "";
      }
      if (persist()) alert("Besuchsbericht gespeichert.");
    };
    $("#saveVisitComparison").onclick = () => {
      for (const key of [
        "beforeProduct",
        "afterProduct",
        "beforeDose",
        ...
        visit.comparison[key] =
          appView.querySelector(`[name="vc.${key}"]`)?.value.trim() || "";
      }
      if (persist()) alert("Vorher-/Nachher-Vergleich gespeichert.");
    };
    $("#findingForm").onsubmit = (e) => {
      e.preventDefault();
      const fd = new FormData(e.currentTarget),
        text = String(fd.get("text") || "").trim();
      if (!text) return;
      const severity = fd.get("severity") || "info";
      visit.findings.unshift({
        id: makeId(),
        severity,
        text,
        createdAt: new Date().toISOString(),
    ...
    $$("[data-remove-finding]").forEach(
      (b) =>
        (b.onclick = () => {
          visit.findings = visit.findings.filter(
            (f) => f.id !== b.dataset.removeFinding,
          );
          if (persist()) render();
        }),
    );
    $("#saveVisitSummary").onclick = () => {
      visit.summary = $("#visitSummary").value.trim();
      visit.notes = visit.summary;
      if (persist()) alert("Besuchsnotiz gespeichert.");
    };
    $("#visitPhotoInput").onchange = async (e) => {
      const files = [...e.target.files].slice(
        0,
        Math.max(0, 6 - visit.photos.length),
      );
      for (const file of files) {
        if (file.size > 1500000) {
          alert(
            `${file.name}: Foto ist größer als 1,5 MB und wurde nicht gespeichert.`,
          );
          continue;
        }
      ...
      if (files.length) visit.checklist.photos = true;
      if (persist()) render();
    };
    $$("[data-remove-photo]").forEach(
      (b) =>
        (b.onclick = () => {
          visit.photos = visit.photos.filter(
            (ph) => ph.id !== b.dataset.removePhoto,
          );
          if (!visit.photos.length) visit.checklist.photos = false;
          if (persist()) render();
        }),
    );
  };
...
function showDewateringForm() {
  const plant = activePlant();
  if (!plant) return showPlantForm();
  const d = dewateringDefaults(plant.sludgeDewatering || {});
  setView("plantDashboard");
  setBreadcrumb(
    `Anlagen › ${plant.master.name || "Unbenannte Anlage"} › Schlammentwässerung`,
  );
  appView.innerHTML = `<form id="dewateringForm" class="record-form"><section class="page-header"><div><p class="eyebrow">Technikakte</p><h1>Schlammentwässerung</h1><p class="subtitle">${esc(plant.master.name || "")}</p></div></section><section class="form-section"><div class="toggle-panel">${checkboxField("enabled", "Schlammentwässerung vorhanden", d.enabled)}</div><div class="form-grid" id="standaloneDewateringFields">${selectField(
  ...
  )}${selectField("process", "Verfahren", d.process, [
    ["screw-press", "Schneckenpresse"],
    ["belt-press", "Siebbandpresse"],
    ["filter-press", "Kammerfilterpresse"],
    ["centrifuge", "Zentrifuge"],
    ...
    ["dryingBed", "Trockenbeet"],
    ["other", "Sonstiges"],
  ])}${field("manufacturer", "Hersteller", d.manufacturer)}${field("model", "Typ / Modell", d.model)}${field("year", "Baujahr", d.year, "number")}${field("unitCount", "Anzahl Aggregate", d.unitCount, "number")}${selectField(
    "operationMode",
    "Betriebsweise",
    d.operationMode,
    [
...
function showDosingForm() {
  const plant = activePlant();
  if (!plant) return showPlantForm();
  let items = (plant.dosingSystems || []).map(dosingDefaults);
  setView("plantDashboard");
  setBreadcrumb(
    `Anlagen › ${plant.master.name || "Unbenannte Anlage"} › Dosiertechnik`,
  );
  ...
  const form = $("#dosingForm"),
    editor = $("#standaloneDosingEditor");
  const sync = () => {
    items = items.map((cur, i) => {
      const out = dosingDefaults({ id: cur.id });
      for (const k of Object.keys(out)) {
        if (k === "id") continue;
        const el = form.elements.namedItem(`d.${i}.${k}`);
        if (el) out[k] = typeof out[k] === "boolean" ? el.checked : el.value;
        else out[k] = cur[k];
      }
      return out;
    });
  ...
  const render = () => {
    editor.innerHTML = items.length
      ? items
          .map(
            (d, i) =>
              `<section class="form-section"><div class="section-heading"><h2>${esc(d.name || `Dosierstation ${i + 1}`)}</h2><button type="button" class="danger-link" data-remove-dose="${i}">Entfernen</button></div><div class="form-grid">${field(`d.${i}.name`, `Bezeichnung`, d.name)}${selectField(
                `d.${i}.purpose`,
                `Verwendungszweck`,
                d.purpose,
                [
              ...
              )}${selectField(`d.${i}.status`, `Betriebsstatus`, d.status, [
                ["active", "In Betrieb"],
                ["inactive", "Außer Betrieb"],
                ["reserve", "Reserve"],
                ["planned", "Geplant"],
              ])}${field(`d.${i}.location`, `Einbauort`, d.location)}${field(`d.${i}.stationManufacturer`, `Hersteller`, d.stationManufacturer)}${field(`d.${i}.stationModel`, `Typ`, d.stationModel)}${field(`d.${i}.stationYear`, `Baujahr`, d.stationYear, "number")}${selectField(
                `d.${i}.pumpType`,
                `Pumpenart`,
                d.pumpType,
                [
              ...
              )}${field(`d.${i}.pumpCount`, `Anzahl Pumpen`, d.pumpCount, "number")}${field(`d.${i}.capacityLh`, `Förderleistung [l/h]`, d.capacityLh, "number")}${field(`d.${i}.maxPressureBar`, `Maximaldruck [bar]`, d.maxPressureBar, "number")}${selectField(
                `d.${i}.controlMode`,
                `Betriebsweise`,
                d.controlMode,
                [
  ...
  render();
  $("#addStandaloneDosing").onclick = () => {
    sync();
    items.push(dosingDefaults({ name: `Dosierstation ${items.length + 1}` }));
    render();
  };
  $("#cancelTechnical").onclick = showPlantDashboard;
  form.onsubmit = (e) => {
...
function showTankForm() {
  const plant = activePlant();
  if (!plant) return showPlantForm();
  let items = (plant.tankSystems || []).map(tankDefaults);
  setView("plantDashboard");
  setBreadcrumb(
    `Anlagen › ${plant.master.name || "Unbenannte Anlage"} › Tankanlagen`,
  );
  ...
  const sync = () => {
    items = items.map((cur, i) => {
      const out = tankDefaults({ id: cur.id });
      for (const k of Object.keys(out)) {
        if (k === "id") continue;
        const el = form.elements.namedItem(`t.${i}.${k}`);
        if (el) out[k] = typeof out[k] === "boolean" ? el.checked : el.value;
        else out[k] = cur[k];
      }
      return out;
  ...
  const render = () => {
    editor.innerHTML = items.length
      ? items
          .map(
            (t, i) =>
              `<section class="form-section"><div class="section-heading"><h2>${esc(t.name || `Tankanlage ${i + 1}`)}</h2><button type="button" class="danger-link" data-remove-tank="${i}">Entfernen</button></div><div class="form-grid">${field(`t.${i}.name`, `Bezeichnung`, t.name)}${selectField(
                `t.${i}.status`,
                `Betriebsstatus`,
                t.status,
                [
              ...
              )}${field(`t.${i}.location`, `Standort`, t.location)}${selectField(
                `t.${i}.type`,
                `Tankart`,
                t.type,
                [
              ...
              )}${field(`t.${i}.manufacturer`, `Hersteller`, t.manufacturer)}${field(`t.${i}.model`, `Typ`, t.model)}${field(`t.${i}.year`, `Baujahr`, t.year, "number")}${field(`t.${i}.volume`, `Volumen [l]`, t.volume, "number")}${field(`t.${i}.material`, `Material`, t.material)}${field(`t.${i}.medium`, `Medium / Produkt`, t.medium)}${field(`t.${i}.lastInspection`, `Letzte Prüfung`, t.lastInspection, "date")}${field(`t.${i}.nextInspection`, `Nächste Prüfung`, t.nextInspection, "date")}<div class="span-2 check-grid">${checkboxField(`t.${i}.doubleWalled`, `Doppelwandig`, t.doubleWalled)}${checkboxField(`t.${i}.bundPresent`, `Auffangwanne`, t.bundPresent)}${checkboxField(`t.${i}.levelMonitoring`, `Füllstandsüberwachung`, t.levelMonitoring)}${checkboxField(`t.${i}.leakageMonitoring`, `Leckageüberwachung`, t.leakageMonitoring)}</div><label class="field-label span-2">Bemerkungen<textarea name="t.${i}.notes">${esc(t.notes)}</textarea></label></div></section>`,
          )
          .join("")
      : `<div class="empty-panel"><p>Noch keine Tankanlage angelegt.</p></div>`;
    editor.querySelectorAll("[data-remove-tank]").forEach(
      (b) =>
        (b.onclick = () => {
          sync();
          items.splice(Number(b.dataset.removeTank), 1);
          render();
        }),
    );
    enableDecimalInputs(editor);
  ...
  $("#addTank").onclick = () => {
    sync();
    items.push(tankDefaults({ name: `Tankanlage ${items.length + 1}` }));
    render();
  };
  $("#cancelTechnical").onclick = showPlantDashboard;
  form.onsubmit = (e) => {
  ...
  (plant.tankSystems || []).forEach((t) => {
    if (t.nextInspection) {
      ...
      if (days < 0)
        items.push({
          level: "red",
          title: `Tankprüfung überfällig`,
          text: `${t.name || t.medium || "Tankanlage"}: seit ${Math.abs(days)} Tagen fällig`,
          component: "Tankanlage",
        });
      else if (days <= 90)
        items.push({
          level: "yellow",
          title: `Tankprüfung steht an`,
          text: `${t.name || t.medium || "Tankanlage"}: in ${days} Tagen`,
          component: "Tankanlage",
        });
    }
    if (t.medium && !t.bundPresent)
      items.push({
        level: "yellow",
        title: "Auffangwanne prüfen",
        text: `${t.name || t.medium}: keine Auffangwanne dokumentiert`,
        component: "Tankanlage",
      });
  });
  (plant.dosingSystems || []).forEach((d) => {
    if (d.hazardous && !d.safetyDataSheetAvailable)
      items.push({
        level: "red",
        title: "Sicherheitsdatenblatt fehlt",
        text: `${d.name || d.productName || "Dosierstation"}`,
        component: "Dosiertechnik",
      });
    if (Number(d.pumpCount || 0) === 1 && !d.standbyPump)
      items.push({
        level: "yellow",
        title: "Keine Reservepumpe dokumentiert",
        text: `${d.name || "Dosierstation"}`,
        component: "Dosiertechnik",
      });
  });
  const dw = plant.sludgeDewatering || {};
  if (dw.enabled && (!dw.outletTsPercent || !dw.polymerKgPerTds))
    items.push({
      level: "yellow",
      title: "Entwässerungsdaten unvollständig",
      text: "Kuchen-TS oder Polymerverbrauch fehlen.",
      component: "Schlammentwässerung",
    });
  return items;
}
function openPlantActions(plant) {
  return (plant.actions || []).filter((a) => a.status !== "done");
}
function renderTodayCockpit(plant) {
  const primary = plant.contacts?.[0];
  const visits = [...(plant.visits || [])].sort((a, b) =>
    String(b.start || b.startedAt).localeCompare(
      String(a.start || a.startedAt),
    ),
  );
  const last = visits.find(
    (v) => v.modeStatus === "completed" || v.status === "done",
  );
  ...
  return `<section class="today-cockpit">
    ...
    ${
      diagnostics.length
        ? `<div class="today-alerts">${diagnostics
            .slice(0, 4)
            .map(
              (x) =>
                `<article class="today-alert ${x.level}"><b>${x.level === "red" ? "!" : "•"}</b><div><strong>${esc(x.title)}</strong><span>${esc(x.text)}</span></div></article>`,
            )
            .join("")}</div>`
        : `<div class="today-ok">Keine automatischen Hinweise aus den vorhandenen Technikdaten.</div>`
    }
  </section>`;
}
function renderActionCenter(plant) {
  const actions = [...(plant.actions || [])].sort(
    (a, b) =>
...
function renderPlantTimeline(plant) {
  const entries = [];
  (plant.visits || []).forEach((v) =>
    entries.push({
      date: v.completedAt || v.start || v.startedAt,
      type: "visit",
      title: v.title || "Besuch",
      text:
        v.summary ||
        v.purpose ||
        `${v.findings?.length || 0} Auffälligkeiten · ${v.photos?.length || 0} Fotos`,
    }),
  );
  (plant.actions || [])
    .filter((a) => a.status === "done")
  ...
  return `<section class="dashboard-section"><div class="section-heading"><div><p class="eyebrow">Entwicklung der Anlage</p><h2>Zeitleiste</h2></div></div><div class="plant-timeline">${
    entries.length
      ? entries
          .slice(0, 12)
          .map(
            (e) =>
              `<article><div class="timeline-dot ${e.type}"></div><div><time>${formatDateTime(e.date)}</time><strong>${esc(e.title)}</strong><p>${esc(e.text)}</p></div></article>`,
          )
          .join("")
      : `<div class="empty-panel compact"><p>Noch keine Ereignisse vorhanden.</p></div>`
  }</div></section>`;
}
...
function plantPassData(plant) {
  const completedVisits = (plant.visits || []).filter(
    (v) => v.modeStatus === "completed" || v.status === "done",
  );
  const visitPhotos = (plant.visits || []).reduce(
    (sum, v) => sum + (v.photos?.length || 0),
    0,
  );
  const technicalCount =
    (plant.sludgeDewatering?.enabled ? 1 : 0) +
    (plant.dosingSystems?.length || 0) +
    (plant.tankSystems?.length || 0);
  const technicalDetails = [
    [
      hasValue(plant.master.mainProcess) || hasValue(plant.master.process),
      "Hauptverfahren",
    ],
    [technicalCount > 0, "Mindestens eine technische Komponente"],
    [
      !plant.sludgeDewatering?.enabled ||
        hasValue(plant.sludgeDewatering.manufacturer),
      "Hersteller Schlammentwässerung",
    ],
    [
      !(plant.dosingSystems || []).length ||
        (plant.dosingSystems || []).every(
          (x) => hasValue(x.name) && hasValue(x.purpose),
        ),
      "Bezeichnung und Zweck der Dosierstationen",
    ],
    [
      !(plant.tankSystems || []).length ||
        (plant.tankSystems || []).every(
          (x) => hasValue(x.name) && hasValue(x.capacity),
        ),
      "Bezeichnung und Volumen der Tankanlagen",
    ],
  ];
  const sections = [
    {
      ...
      weight: 24,
      score: scoreFields([
        [plant.master.name, "Anlagenname"],
        [plant.master.internalNumber, "Anlagennummer"],
        [plant.master.type, "Anlagentyp"],
        [plant.master.capacityPE, "Ausbaugröße"],
        [plant.master.actualPE, "tatsächliche Belastung"],
        [plant.master.mainProcess || plant.master.process, "Hauptverfahren"],
      ]),
    },
    {
      key: "location",
      label: "Standort & Zugang",
      weight: 16,
      score: scoreFields([
        [plant.address.street, "Straße"],
        [plant.address.postalCode, "Postleitzahl"],
        [plant.address.city, "Ort"],
        [plant.address.latitude || plant.address.gps, "GPS-Koordinaten"],
        [plant.access.parking, "Parkhinweis"],
        [plant.access.registration, "Anmeldung vor Ort"],
      ]),
    },
    {
      key: "contacts",
      label: "Betreiber & Kontakte",
      weight: 16,
      score: scoreFields([
        [plant.operator.name, "Betreiber"],
        [plant.operator.customerNumber, "Kundennummer"],
        [plant.operator.phone || plant.operator.email, "Kontakt Betreiber"],
        [plant.contacts?.[0]?.name, "Hauptansprechpartner"],
        [
          plant.contacts?.[0]?.mobile ||
            plant.contacts?.[0]?.phone ||
            plant.contacts?.[0]?.email,
    ...
    },
    {
      ...
      label: "Technik",
      weight: 22,
      score: scoreFields(technicalDetails),
    },
    {
      key: "operations",
      label: "Betriebswerte",
      weight: 12,
      score: scoreFields([
        [plant.parameters.flow, "Volumenstrom"],
        [plant.parameters.pOut, "Pges Ablauf"],
        [plant.parameters.nh4Out, "NH₄-N Ablauf"],
        [plant.parameters.svi, "SVI"],
        [plant.parameters.cakeTs, "Kuchen-TS"],
        [plant.parameters.polymer, "Polymerverbrauch"],
      ]),
    },
    {
      key: "history",
      ...
      weight: 10,
      score: scoreFields([
        [completedVisits.length, "abgeschlossener Besuch"],
        [visitPhotos, "Fotodokumentation"],
        [(plant.actions || []).length, "Aufgabenhistorie"],
      ]),
    },
  ];
  const overall = Math.round(
    sections.reduce((sum, s) => sum + s.score.percent * s.weight, 0) /
      sections.reduce((sum, s) => sum + s.weight, 0),
  );
  const missing = sections
    .flatMap((s) => s.score.missing.map((item) => ({ section: s.label, item })))
    .slice(0, 8);
  const diagnostics = plantDiagnostics(plant);
  return {
    overall,
...
function renderDigitalPlantPass(plant) {
  const pass = plantPassData(plant),
    status = passStatus(pass.overall);
  return `<section class="plant-pass">
    ...
    <div class="pass-detail-grid">
      ...
      <article class="pass-diagnostics"><h3>Wartung und Hinweise</h3>${
        pass.diagnostics.length
          ? `<ul>${pass.diagnostics
              .slice(0, 6)
              .map(
                (x) =>
                  `<li class="${x.level}"><span>${esc(x.component)}</span><strong>${esc(x.title)}</strong><small>${esc(x.text)}</small></li>`,
              )
              .join("")}</ul>`
          : '<div class="empty-panel compact"><p>Aktuell keine automatischen Hinweise.</p></div>'
      }</article>
    </div>
  </section>`;
}
...
function plantHeader(plant) {
  const type =
    plant.master.type === "industrial"
      ? "Industrielle Kläranlage"
      : plant.master.type === "mixed"
        ? "Kommunale Kläranlage mit Industrieanteil"
        : "Kommunale Kläranlage";
...
function renderPlantTechnologyPage(plant) {
  ...
  <div class="kpi-grid">${[
    ["Volumenstrom", plant.parameters.flow, "m³/d"],
    ["Pges Ablauf", plant.parameters.pOut, "mg/l"],
    ["NH₄-N Ablauf", plant.parameters.nh4Out, "mg/l"],
    ["SVI", plant.parameters.svi, "ml/g"],
    ["Schlammalter", plant.parameters.sludgeAge, "d"],
    ["Kuchen-TS", plant.parameters.cakeTs, "%"],
    ["Feststoffrückhalt", plant.parameters.retention, "%"],
    ["Polymer", plant.parameters.polymer, "kg WS/t TS"],
  ]
    .map(
      ([l, v, u]) =>
        `<article class="kpi-card"><span>${l}</span><strong>${fmt(v)}</strong><small>${u}</small></article>`,
    )
    .join("")}</div></section>
...
function renderPlantTasksPage(plant) {
  return renderActionCenter(plant);
}
...
function renderPlantSystemPage(plant) {
  const visits = (plant.visits || []).length,
    actions = (plant.actions || []).length;
...
function renderPlantPage(plant, page) {
  const renderers = {
    ...
    tasks: renderPlantTasksPage,
    record: renderPlantRecordPage,
  };
...
function showPlantDashboard(page) {
  ...
  page = valid.has(page)
    ...
    : valid.has(localStorage.getItem(STORAGE_PLANT_PAGE))
      ? localStorage.getItem(STORAGE_PLANT_PAGE)
      : "overview";
  localStorage.setItem(STORAGE_PLANT_PAGE, page);
  setView("plantDashboard");
  setBreadcrumb(`Anlagen › ${plant.master.name || "Unbenannte Anlage"}`);
  ...
  $$("[data-plant-page]").forEach(
    (b) => (b.onclick = () => showPlantDashboard(b.dataset.plantPage)),
  );
  $$("[data-jump-page]").forEach(
    (b) => (b.onclick = () => showPlantDashboard(b.dataset.jumpPage)),
  );
  ...
  $("#openNavigation")?.addEventListener("click", () => {
    const url = googleMapsUrls(plant).directions;
    if (locationQuery(plant)) window.open(url, "_blank", "noopener");
    else alert("Bitte zuerst eine Adresse oder GPS-Koordinaten hinterlegen.");
  });
  $("#startVisitCockpit")?.addEventListener("click", () => showVisitMode());
  $("#completePlantPass")?.addEventListener("click", () =>
    showPlantForm(plant.id),
  );
  $("#editDewatering")?.addEventListener("click", showDewateringForm);
  $("#editDosing")?.addEventListener("click", showDosingForm);
  $("#editTanks")?.addEventListener("click", showTankForm);
  $("#editParameters")?.addEventListener("click", () =>
    showPlantForm(plant.id),
  );
  $("#openTraffic")?.addEventListener("click", showTraffic);
  ...
  $$(`[data-toggle-action]`).forEach(
    (b) =>
      (b.onclick = () => {
        const a = (plant.actions || []).find(
          (x) => x.id === b.dataset.toggleAction,
        );
        if (!a) return;
        a.status = a.status === "done" ? "open" : "done";
        a.completedAt = a.status === "done" ? new Date().toISOString() : "";
        if (savePlants()) showPlantDashboard("tasks");
      }),
  );
  $$(`[data-delete-action]`).forEach(
    (b) =>
      (b.onclick = () => {
        if (!confirm("Aufgabe wirklich löschen?")) return;
        plant.actions = (plant.actions || []).filter(
          (a) => a.id !== b.dataset.deleteAction,
        );
        if (savePlants()) showPlantDashboard("tasks");
      }),
  );
  $$("[data-open-visit]").forEach(
    (b) => (b.onclick = () => showVisitMode(b.dataset.openVisit)),
  );
  $$("[data-edit-visit]").forEach(
    (b) => (b.onclick = () => showVisitForm(b.dataset.editVisit)),
  );
  $$("[data-ics-visit]").forEach(
    (b) =>
      (b.onclick = () => {
        const v = (plant.visits || []).find((x) => x.id === b.dataset.icsVisit);
        if (v) exportVisitIcs(plant, v);
      }),
  );
  $$("[data-delete-visit]").forEach(
    (b) =>
      (b.onclick = () => {
        const v = (plant.visits || []).find(
          (x) => x.id === b.dataset.deleteVisit,
        );
        if (confirm(`Termin „${v?.title || "Besuch"}“ wirklich löschen?`)) {
          plant.visits = (plant.visits || []).filter(
            (x) => x.id !== b.dataset.deleteVisit,
          );
          savePlants();
          showPlantDashboard("visits");
        }
      }),
  );
...
function showLimits() {
  ...
  if (!plant) return showPlantForm();
  setView("limits");
  setBreadcrumb("Grenz- und Zielwerte");
  ...
  <form id="limitsForm" class="limits-list">${limits
    .map(
      (l, i) => `<article class="limit-card">
    ...
    .join(
      "",
    )}<div class="sticky-form-actions"><button type="button" class="button secondary" id="resetLimits">Standardwerte</button><button class="button primary" type="submit">Grenzen speichern</button></div></form>`;
  enableDecimalInputs(appView);
  $("#resetLimits").onclick = () => {
    plant.limits = structuredClone(defaultLimits);
    savePlants();
    showLimits();
  };
  $("#limitsForm").onsubmit = (e) => {
    ...
    limits.forEach((l, i) => {
      const props =
        l.direction === "range"
          ? ["greenMin", "greenMax", "warningMin", "warningMax"]
          : ["target", "warning", "legal"];
      props.forEach((prop) => {
        const v = fd.get(`limit.${i}.${prop}`);
        l[prop] = v === "" ? null : Number(v);
      });
    });
    plant.limits = limits;
    plant.updatedAt = new Date().toISOString();
...
function showTraffic() {
  ...
  <div class="traffic-grid">${evals
    .map(
      (item) => `<article class="traffic-card ${item.evaluation.level}">
...
function showProfile() {
  setView("profile");
  setBreadcrumb("Mitarbeiterprofil");
  const p = employeeProfile,
    full = employeeDisplayName();
  ...
  $("#editEmployeeProfile").onclick = showProfileForm;
  $("#downloadVCard").onclick = downloadVCard;
  ...
  $("#importFullBackup").onchange = async (e) => {
    const file = e.target.files?.[0];
    if (!file) return;
    try {
      const data = JSON.parse(await file.text());
      ...
      activePlantId =
        data.activePlantId && plants.some((x) => x.id === data.activePlantId)
          ? data.activePlantId
          : plants[0]?.id || "";
      if (savePlants() && saveEmployeeProfile()) showProfile();
    } catch (err) {
      alert(`Import nicht möglich: ${err.message}`);
    } finally {
      e.target.value = "";
...
function showProfileForm() {
  setView("profileForm");
  setBreadcrumb("Mitarbeiterprofil › Bearbeiten");
  const p = employeeProfile;
  ...
  const form = $("#employeeProfileForm");
  $("#cancelEmployeeProfile").onclick = showProfile;
  form.onsubmit = (e) => {
    e.preventDefault();
    const fd = new FormData(form),
      next = normalizeEmployeeProfile();
    for (const key of Object.keys(next)) {
      if (key === "schemaVersion" || key === "mobile" || key === "phone")
        continue;
      if (fd.has(key)) next[key] = String(fd.get(key) || "").trim();
    }
    next.mobile = combinePhone(fd, "mobile");
    next.phone = combinePhone(fd, "phone");
    employeeProfile = next;
    if (saveEmployeeProfile()) showProfile();
  };
}
...
$$("[data-global-view]").forEach(
  (button) =>
    (button.onclick = () => {
      showGlobalPage(button.dataset.globalView);
      closeMobileSidebar();
    }),
);
const showAllButton = $("#showAllCalculators");
if (showAllButton)
...
$("#breadcrumbHome").onclick = () => showGlobalPage("today");
...
$("#activePlantSelect").onchange = (e) => {
  activePlantId = e.target.value;
  savePlants();
  showPlantDashboard();
};
$("#managePlantsButton").onclick = () => {
  showApplication("plants");
  closeMobileSidebar();
};
$("#newPlantButton").onclick = () => {
  showPlantForm();
  closeMobileSidebar();
};
$$("[data-view]").forEach(
  (b) =>
    (b.onclick = () => {
      showApplication(b.dataset.view);
      closeMobileSidebar();
    }),
);
$$("[data-static-toggle]").forEach(
  (b) =>
    (b.onclick = () => {
      const group = b.closest(".menu-group");
      group.classList.toggle("open");
      b.setAttribute("aria-expanded", group.classList.contains("open"));
    }),
);
...
$("#clearSearch").onclick = () => {
  $("#searchInput").value = "";
  state.query = "";
  showGlobalPage("today");
};
$("#favoriteFilter").onclick = () => {
  state.favoritesOnly ? showHome() : showFavorites();
  closeMobileSidebar();
};
$("#exportPlantButton").onclick = () => {
  const plant = activePlant();
  if (!plant) return alert("Bitte zuerst eine Anlage auswählen.");
  const safe = (plant.master.name || "klaeranlage")
    .toLowerCase()
    .replace(/[^a-z0-9äöüß]+/gi, "-")
    .replace(/^-|-$/g, "");
  downloadJson(`${safe || "klaeranlage"}-anlagenakte.json`, {
    schema: "abwasser-rechner-plant-v1",
    exportedAt: new Date().toISOString(),
    plant,
  });
};
$("#importPlantInput").onchange = async (e) => {
  ...
  try {
    ...
    const imported = parsed.plant || parsed;
    if (!imported.master || !imported.address || !imported.operator)
      throw new Error("Ungültige Anlagenakte");
    imported.id = crypto.randomUUID();
    imported.createdAt = new Date().toISOString();
    imported.updatedAt = new Date().toISOString();
    imported.limits = imported.limits || structuredClone(defaultLimits);
    imported.contacts = imported.contacts || [];
    imported.parameters = imported.parameters || {};
    imported.access = imported.access || {};
    imported.visits = imported.visits || [];
    plants.push(imported);
    activePlantId = imported.id;
    savePlants();
    showPlantDashboard();
  } catch (err) {
    alert(`Import nicht möglich: ${err.message}`);
...
async function bootstrap() {
  try {
    ...
    if (migration.migrated)
      console.info(
        `Migration: ${migration.migrated} Dokumente, ${migration.files || 0} PDFs`,
      );
  } catch (error) {
    console.error("Datenbankstart fehlgeschlagen", error);
    alert(
      "Die lokale Datenbank konnte nicht initialisiert werden: " +
        (error?.message || error),
    );
  }
  renderPlantSelector();
  renderCategoryMenu();
  updateProfileButton();
  showGlobalPage(localStorage.getItem(STORAGE_GLOBAL_PAGE) || "today");
}
bootstrap();
