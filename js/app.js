...
function normalizeDocument(d={}){return {id:d.id||makeId(),fileName:d.fileName||"",mimeType:d.mimeType||"application/pdf",size:Number(d.size)||0,importedAt:d.importedAt||new Date().toISOString(),updatedAt:d.updatedAt||new Date().toISOString(),type:d.type||"other",status:d.status||"imported",documentNumber:d.documentNumber||"",documentDate:d.documentDate||"",version:d.version||"",language:d.language||"DE",sender:d.sender||"",recipient:d.recipient||"",customer:d.customer||"",plantId:d.plantId||"",project:d.project||"",productIds:Array.isArray(d.productIds)?d.productIds:[],tags:Array.isArray(d.tags)?d.tags:[],notes:d.notes||"",rawText:d.rawText||"",textExtracted:Boolean(d.textExtracted),reviewedAt:d.reviewedAt||"",reviewer:d.reviewer||"",extracted:d.extracted||{},source:d.source||"PDF-Import"}}
function loadDocuments(){try{const x=JSON.parse(localStorage.getItem(STORAGE_DOCUMENTS)||"[]");return Array.isArray(x)?x.map(normalizeDocument):[]}catch{return []}}
let documents=loadDocuments();
function saveDocuments(){try{localStorage.setItem(STORAGE_DOCUMENTS,JSON.stringify(documents));return true}catch(e){console.error(e);alert("Dokumentmetadaten konnten nicht gespeichert werden.");return false}}
...
async function importDocuments(files){const pdfs=[...files].filter(f=>f.type==="application/pdf"||/\.pdf$/i.test(f.name));if(!pdfs.length)return alert("Bitte mindestens eine PDF-Datei auswählen.");for(const file of pdfs){const id=makeId();try{await storeProductFile(id,file);const rawText=await extractPdfTextBasic(file);const type=detectGeneralDocumentType(rawText,file.name);documents.unshift(normalizeDocument({id,fileName:file.name,mimeType:file.type||"application/pdf",size:file.size,type,status:"imported",rawText,textExtracted:Boolean(rawText),extracted:inferProductFromPdf(rawText,file.name,type)}));saveDocuments()}catch(e){console.error(e);alert(`Datei konnte nicht offline gespeichert werden: ${file.name}`)}}showDocuments()}
