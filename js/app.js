...
function showPlantDashboard(page){
  ...
  $$(`[data-toggle-action]`).forEach(b=>b.onclick=()=>{const a=(plant.actions||[]).find(x=>x.id===b.dataset.toggleAction);if(!a)return;const wasDone=a.status==="done";a.status=a.status==="done"?"open":"done";a.completedAt=a.status==="done"?new Date().toISOString():"";if(!savePlants())return;showPlantDashboard("tasks");if(a.status==="done"&&!wasDone)showCompletionFollowUpDialog({plant,sourceType:"task",source:a});});
