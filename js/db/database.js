...
export const STORES=Object.freeze({
  ...
  organizations:'organizations',memberships:'memberships',organizationSettings:'organizationSettings'
});
let dbPromise;
...
export async function getOne(store,id){const db=await openDatabase();return new Promise((resolve,reject)=>{const r=db.transaction(store,'readonly').objectStore(store).get(id);r.onsuccess=()=>resolve(r.result||null);r.onerror=()=>reject(r.error)})}
