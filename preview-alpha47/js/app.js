...
function createBuiltInDemoPlant(){
  const now=new Date();
  const createdAt=now.toISOString();
  const demoDateTime=(days,hours,minutes)=>{
  ...
  return normalizePlant({
    ...
    access:{parking:"Besucherparkplatz vor dem Betriebsgebäude",gate:"Haupttor",registration:"Im Betriebsgebäude beim Betriebsleiter anmelden"},
    ...
    tankSystems:[{
      name:"Fällmitteltank 1",status:"active",type:"storage-tank",year:"2018",volume:"20000",material:"PE-HD",medium:"PAC",levelMonitoring:true,leakageMonitoring:true
    }],
    dosingSystems:[{
      name:"Phosphatfällung",purpose:"precipitant",status:"active",location:"Chemikalienraum",pumpType:"diaphragm",pumpCount:"2",capacityLh:"120",
      controlMode:"flow-proportional",standbyPump:true,productName:"PAC",flowMeter:true,levelSensor:true,dryRunProtection:true
    }],
    actions:[
      {id:makeId(),title:"Polymerverbrauch der Schlammentwässerung prüfen",status:"open",priority:"normal",dueDate:isoDateOffset(7),component:"",sourceVisitId:"",createdAt,completedAt:"",autoGenerated:false,followUpType:"",followUpSourceId:""},
      {id:makeId(),title:"Fällmitteltank und Dosierung kontrollieren",status:"open",priority:"high",dueDate:isoDateOffset(14),component:"",sourceVisitId:"",createdAt,completedAt:"",autoGenerated:false,followUpType:"",followUpSourceId:""}
    ],
    visits:[
      {title:"Prozessoptimierung Schlammentwässerung",start:demoDateTime(3,9,0),end:demoDateTime(3,11,0),visitType:"process-optimization",processArea:"dewatering",purpose:"Polymerverbrauch und Kuchen-TS überprüfen",contact:"Max Mustermann",status:"planned"},
      {title:"Nachkontrolle Phosphatfällung",start:demoDateTime(10,10,0),end:demoDateTime(10,11,0),visitType:"follow-up",processArea:"phosphorus",purpose:"Dosierung und Ablaufwerte kontrollieren",contact:"Max Mustermann",status:"planned"}
    ]
  });
}
function ensureBuiltInDemoPlant(){
  ...
  plants.push(demoPlant);
  if(!activePlantId||!plants.some(p=>p.id===activePlantId))activePlantId=demoPlant.id;
  savePlants();
}
...
function showGlobalPage(page){
  ...
  if(page==="appointments"){
    ...
    appView.innerHTML=`${globalPageHeader("Arbeitsliste","Aufgaben","Offene Aufgaben aus allen Anlagen.")}<div class="global-task-list">${tasks.length?tasks.map(({plant,action})=>renderGlobalTaskCard(plant,action)).join(""):`<div class="empty-panel"><h2>Keine offenen Aufgaben</h2><p>Offene Punkte aus Besuchen und Anlagenakten werden hier gesammelt.</p></div>`}</div>`;
    bindTaskActions();
    $$('[data-toggle-action]').forEach(b=>b.onclick=()=>{
      const card=b.closest('[data-plant-id]');
      const p=plants.find(x=>x.id===card?.dataset.plantId);
      const a=p&&(p.actions||[]).find(x=>x.id===b.dataset.toggleAction);
      if(!p||!a)return;
      const wasDone=a.status==="done";
      a.status=wasDone?"open":"done";
      a.completedAt=a.status==="done"?new Date().toISOString():"";
      if(wasDone)a.completionOutcome="";
      p.updatedAt=new Date().toISOString();
      if(!savePlants())return;
      showGlobalPage("tasks-global");
      if(a.status==="done"&&!wasDone)showCompletionFollowUpDialog({plant:p,sourceType:"task",source:a});
    });
  }else if(page==="backup"){
...
function renderPlants(){
  ...
  <div class="plant-grid">${plants.length?plants.map(p=>`<article class="plant-card ${p.id===activePlantId?"active":""}">
    <div class="plant-card-head"><span class="plant-type">${p.master.type==="industrial"?"Industriell":p.master.type==="mixed"?"Kommunal mit Industrieanteil":"Kommunal"}</span>${p.id===BUILT_IN_DEMO_PLANT_ID?`<span class="active-chip">Testanlage</span>`:""}${p.id===activePlantId?`<span class="active-chip">Aktiv</span>`:""}</div>
...
function taskTypeMeta(type){
  return TASK_TYPES[type]||TASK_TYPES.general;
}
function taskContactRecord(plant,action){
  const contacts=plant.contacts||[];
  if(!contacts.length)return null;
  const name=String(action.contactName||"").trim();
  if(name){
    const match=contacts.find(c=>String(c.name||"").trim()===name);
    if(match)return match;
  }
...
function taskPhone(plant,action){
  const contact=taskContactRecord(plant,action);
  return contact?.mobile||contact?.phone||plant.operator?.phone||"";
}
function taskEmail(plant,action){
  const contact=taskContactRecord(plant,action);
  return contact?.email||plant.operator?.email||"";
}
...
function createFollowUpAction(plant,payload){
  ...
  plant.actions=[...(plant.actions||[]),{
    ...
    createdAt:new Date().toISOString(),
    completedAt:"",
    autoGenerated:payload.autoGenerated!==undefined?payload.autoGenerated:true,
    followUpType,
    followUpSourceId,
...
function suggestTask(id,label,description,{offsetDays,taskTitle,followUpType,taskType="general",recommended=true}={}){
  return {id,label,description,recommended,actionType:"task",offsetDays,taskTitle,followUpType,taskType};
}
...
function taskCompletionSuggestions(source){
  ...
  if(outcome==="resolved"){
    return [
      suggestTask("review","Wirksamkeit in 14 Tagen prüfen","Kontrollieren, ob die erledigte Maßnahme erfolgreich war.",{offsetDays:14,taskTitle:`Wirksamkeit prüfen: ${title}`,followUpType:"task-review",taskType:"review"}),
  ...
  if(outcome==="further-action"){
    return [
      ...
      suggestTask("progress","Fortschritt in 7 Tagen prüfen","Den Fortschritt der weiteren Maßnahme kontrollieren.",{offsetDays:7,taskTitle:`Fortschritt prüfen: ${title}`,followUpType:"task-progress-review",taskType:"review",recommended:false})
    ];
  }
  if(outcome==="spare-part"){
    return [
      suggestTask("spare-part","Ersatzteil / Angebotsanfrage klären","Material, Ersatzteil oder Angebot zeitnah nachhalten.",{offsetDays:2,taskTitle:`Ersatzteil / Angebot klären: ${title}`,followUpType:"task-spare-part",taskType:"spare-part"}),
      suggestTaskCustom("Weitere Aufgabe anlegen"),
      suggestAppointment("Termin planen")
    ];
  }
  if(outcome==="customer-info"){
    return [
      ...
      suggestTaskCustom("Weitere Aufgabe anlegen"),
      suggestTask("customer-feedback","Rückmeldung in 3 Tagen prüfen","Kontrollieren, ob die Kundenrückmeldung erfolgt ist.",{offsetDays:3,taskTitle:`Kundenrückmeldung prüfen: ${title}`,followUpType:"task-customer-feedback",taskType:"review",recommended:false})
    ];
  }
...
function visitOnSiteSuggestions(source){
  ...
  if(outcome==="goal-achieved"){
    ...
    if(visitType==="product-trial"){
      reviewSuggestion=suggestTask("trial-review","Versuchsauswertung in 7 Tagen","Ergebnisse des Produktversuchs auswerten.",{offsetDays:7,taskTitle:`Versuchsauswertung: ${title}`,followUpType:"visit-trial-review",taskType:"review"});
    }else if(visitType==="technical-service"){
      reviewSuggestion=suggestTask("service-review","Funktionskontrolle in 7 Tagen","Funktion der durchgeführten Servicearbeiten kontrollieren.",{offsetDays:7,taskTitle:`Funktionskontrolle: ${title}`,followUpType:"visit-service-review",taskType:"technical"});
    }else{
      reviewSuggestion=suggestTask("review","Nachkontrolle in 14 Tagen","Ergebnis und Wirksamkeit des Termins prüfen.",{offsetDays:14,taskTitle:`Nachkontrolle: ${title}`,followUpType:"visit-review",taskType:"review"});
    }
  ...
  if(outcome==="offer-needed"){
    return [
      suggestTask("commercial","Angebot / Bestellung klären","Kaufmännischen Handlungsbedarf aus dem Termin nachhalten.",{offsetDays:2,taskTitle:`Angebot / Bestellung prüfen: ${title}`,followUpType:"visit-commercial-follow-up",taskType:"commercial"}),
      suggestTaskCustom("Aufgabe anlegen")
    ];
  }
  return genericFollowUpFallback();
...
function visitCallSuggestions(source){
  ...
  if(outcome==="awaiting-response"){
    return [
      suggestTask("response-review","Rückmeldung in 3 Tagen prüfen","Kontrollieren, ob eine Rückmeldung eingegangen ist.",{offsetDays:3,taskTitle:`Rückmeldung prüfen: ${title}`,followUpType:"call-response-review",taskType:"review"}),
...
function visitSchedulingSuggestions(source){
  ...
  if(outcome==="awaiting-response"){
    return [
      suggestTask("scheduling-review","Terminrückmeldung in 3 Tagen prüfen","Kontrollieren, ob die Terminbestätigung eingegangen ist.",{offsetDays:3,taskTitle:`Terminrückmeldung prüfen: ${title}`,followUpType:"scheduling-response-review",taskType:"review"}),
...
function visitEmailSuggestions(source){
  ...
  if(outcome==="awaiting-response"){
    return [
      suggestTask("email-review","Antwort in 3 Tagen prüfen","Kontrollieren, ob eine Antwort eingegangen ist.",{offsetDays:3,taskTitle:`E-Mail-Rückmeldung prüfen: ${title}`,followUpType:"email-response-review",taskType:"review"}),
...
function visitOtherSuggestions(source){
  ...
  const title=String(source?.title||"Termin").trim();
  if(outcome==="awaiting-response"){
    return [suggestTask("appointment-review","Rückmeldung in 3 Tagen prüfen","Kontrollieren, ob eine Rückmeldung eingegangen ist.",{offsetDays:3,taskTitle:`Rückmeldung prüfen: ${title}`,followUpType:"appointment-response-review",taskType:"review"}),suggestTaskCustom("Aufgabe anlegen")];
  }
  if(outcome==="further-action"){
...
function showCompletionFollowUpDialog({plant,sourceType,source,onClose}){
  ...
  dialog.innerHTML=`<div class="completion-followup-content">
    ...
    <div class="completion-followup-custom" data-followup-custom hidden>
      ...
      <button type="button" class="button primary" data-followup-custom-submit>Aufgabe anlegen</button>
    </div>
    <div class="completion-followup-actions">
      <button type="button" class="button secondary" data-followup-none>Keine Folgeaktivität erforderlich</button>
    </div>
  </div>`;
  ...
  dialog.querySelector("[data-followup-custom-submit]").onclick=()=>{
    ...
    plant.actions=[...(plant.actions||[]),{
      ...
      component:"",
      sourceVisitId:sourceType==="visit"?(source?.id||""):"",
      createdAt:new Date().toISOString(),
      completedAt:"",
      autoGenerated:false,
      followUpType:"manual-follow-up",
      followUpSourceId:source?.id||"",
      taskType:"general",
      contactName:sourceType==="visit"?(source?.contact||""):""
    }];
    savePlants();
    close();
  };
...
function showVisitForm(visitId=null,prefill=null){
  ...
  $("#visitForm").onsubmit=e=>{
    ...
    if(!savePlants())return;
    showPlantDashboard();
    if(saved.status==="done"&&!wasDone)showCompletionFollowUpDialog({plant,sourceType:"visit",source:saved});
  };
}
...
function showVisitMode(visitId=null){
  ...
  const render=()=>{
    ...
    $("#finishVisit").onclick=()=>{const wasDone=visit.status==="done";visit.modeStatus=visit.modeStatus==="completed"?"active":"completed";visit.status=visit.modeStatus==="completed"?"done":"planned";visit.completedAt=visit.modeStatus==="completed"?new Date().toISOString():"";if(!persist())return;render();if(visit.status==="done"&&!wasDone)showCompletionFollowUpDialog({plant,sourceType:"visit",source:visit});};
...
function renderTaskActions(plant,action){
  const type=action.taskType||"general";
  const phone=taskPhone(plant,action);
  const email=taskEmail(plant,action);
  const buttons=[];
  if(type==="call"){
    if(phone)buttons.push(["call","Anrufen"]);
  }else if(type==="scheduling"){
    if(phone)buttons.push(["call","Anrufen"]);
    if(email)buttons.push(["email","E-Mail"]);
    buttons.push(["schedule","Termin eintragen"]);
  }else if(type==="email"){
    if(email)buttons.push(["email","E-Mail schreiben"]);
  }else if(type==="followup"){
    if(phone)buttons.push(["call","Anrufen"]);
    if(email)buttons.push(["email","E-Mail"]);
  }else if(type==="review"){
    buttons.push(["review","Prüfen"]);
  }else if(type==="technical"){
    buttons.push(["technical","Anlage öffnen"]);
  }else if(type==="commercial"){
    buttons.push(["commercial","Anlage öffnen"]);
  }else{
    buttons.push(["open","Anlage öffnen"]);
  }
  const html=buttons.map(([act,label],i)=>`<button type="button" class="${i===0?"task-primary-action":""}" data-task-action="${act}">${label}</button>`).join("");
  const done=action.status==="done";
  return `${html}<button type="button" data-toggle-action="${action.id}">${done?"Wieder öffnen":"Erledigt"}</button>`;
}
function renderTaskCard(plant,action){
  const meta=taskTypeMeta(action.taskType);
  const done=action.status==="done";
  return `<article class="action-item task-type-${action.taskType||"general"} ${done?"done":""} ${action.priority==="high"?"high":""}" data-action-id="${action.id}" data-plant-id="${plant.id}">
    <div class="task-type-badge"><span class="task-type-icon">${meta.icon}</span>${esc(meta.label)}</div>
    <div class="task-content">
      <strong>${esc(action.title)}</strong>
      ${done?`<span class="task-status-label">✓ Erledigt</span>`:""}
      <div class="task-meta">
        ${action.contactName?`<span class="task-contact">${esc(action.contactName)}</span>`:""}
        ${action.component?`<span>${esc(action.component)}</span>`:""}
        ${action.dueDate?`<span>Fällig ${new Date(action.dueDate+'T00:00:00').toLocaleDateString('de-DE')}</span>`:""}
      </div>
    </div>
    <div class="task-actions">${renderTaskActions(plant,action)}<button type="button" class="action-delete" data-delete-action="${action.id}" aria-label="Aufgabe löschen">×</button></div>
  </article>`;
}
function bindTaskActions(){
  $$('[data-action-id]').forEach(card=>{
    const plant=plants.find(p=>p.id===card.dataset.plantId);
    const action=plant&&(plant.actions||[]).find(a=>a.id===card.dataset.actionId);
    if(!plant||!action)return;
    card.querySelectorAll('[data-task-action]').forEach(button=>{
      button.onclick=()=>{
        const act=button.dataset.taskAction;
        if(act==="call"){
          const phone=taskPhone(plant,action);
          if(phone)window.location.href=`tel:${phone.replace(/[^0-9+]/g,"")}`;
        }else if(act==="email"){
          const email=taskEmail(plant,action);
          if(email){
            const subject=encodeURIComponent(action.title||"Rückmeldung");
            const bodyLines=[plant.master.name||"",action.title||""].filter(Boolean);
            const body=bodyLines.length?`&body=${encodeURIComponent(bodyLines.join("\n"))}`:"";
            window.location.href=`mailto:${encodeURIComponent(email)}?subject=${subject}${body}`;
          }
        }else if(act==="schedule"){
          activePlantId=plant.id;savePlants();
          showVisitForm(null,{
            appointmentType:"visit",
            title:action.title||"Termin",
            purpose:`Folgetermin zu: ${action.title||"Aufgabe"}`,
            contact:action.contactName||plant.contacts?.[0]?.name||""
          });
        }else if(act==="review"){
          activePlantId=plant.id;savePlants();showPlantDashboard("overview");
        }else if(act==="technical"){
          activePlantId=plant.id;savePlants();showPlantDashboard("technology");
        }else if(act==="commercial"){
          activePlantId=plant.id;savePlants();showPlantDashboard("sales");
        }else if(act==="open"){
          activePlantId=plant.id;savePlants();showPlantDashboard("overview");
        }
      };
...
function renderActionCenter(plant){
  ...
  return `<section class="dashboard-section action-center"><div class="section-heading"><div><p class="eyebrow">Nachverfolgung</p><h2>Aufgaben und Aktionen</h2></div></div>
    <form id="quickActionForm" class="quick-action-form">
      <select name="taskType"><option value="general">Allgemeine Aufgabe</option><option value="call">Anruf</option><option value="scheduling">Termin vereinbaren</option><option value="email">E-Mail</option><option value="followup">Nachfassen</option><option value="review">Kontrolle</option><option value="technical">Technische Maßnahme</option><option value="commercial">Angebot / Bestellung</option><option value="spare-part">Ersatzteil</option></select>
      <input name="title" required placeholder="Neue Aufgabe, z. B. Polymeroptimierung durchführen">
      <select name="contactName"><option value="">Kein Ansprechpartner</option>${(plant.contacts||[]).map(c=>`<option value="${esc(c.name)}">${esc(c.name)}</option>`).join("")}</select>
      <select name="priority"><option value="normal">Normal</option><option value="high">Hoch</option></select>
      <input name="dueDate" type="date">
      <button class="button primary" type="submit">Aufgabe anlegen</button>
    </form>
    <div class="action-list">${actions.length?actions.map(a=>renderTaskCard(plant,a)).join(''):`<div class="empty-panel compact"><p>Noch keine Aufgaben angelegt.</p></div>`}</div>
  </section>`;
}
function renderGlobalTaskCard(plant,action){
  const meta=taskTypeMeta(action.taskType);
  return `<article class="global-task-card task-type-${action.taskType||"general"} ${action.priority==='high'?'high':''}" data-action-id="${action.id}" data-plant-id="${plant.id}">
    <div class="task-type-badge"><span class="task-type-icon">${meta.icon}</span>${esc(meta.label)}</div>
    <div class="task-content">
      ...
      <div class="task-meta">
        ${action.contactName?`<span class="task-contact">${esc(action.contactName)}</span>`:""}
        <span>${action.dueDate?`Fällig: ${formatDate(action.dueDate)}`:"Ohne Fälligkeit"}</span>
      </div>
    </div>
    <div class="global-task-actions">${renderTaskActions(plant,action)}<button type="button" data-global-open-plant="${plant.id}" data-global-plant-page="tasks">Anlage öffnen</button></div>
  </article>`;
}
...
function plantPageNavigation(active){
  const pages=[["overview","Übersicht"],["technology","Technik"],["visits","Einsätze"],["sales","Vertrieb"],["tasks","Aufgaben"]];
...
function renderPlantMasterDataSummary(plant){
  ...
  <div class="plant-master-summary-grid">
    ...
    <article class="plant-master-card"><h3>Betreiber</h3><dl>${detailRows([["Betreiber",esc(plant.operator?.name)], ["Kundennummer",esc(plant.operator?.customerNumber)], ...(plant.operator?.phone?[['Telefon',telLink(plant.operator.phone)]]:[]), ...(plant.operator?.email?[['E-Mail',mailLink(plant.operator.email)]]:[])])}</dl></article>
    <article class="plant-master-card"><h3>Ansprechpartner</h3><dl>${detailRows([["Name",esc(primary.name)], ["Funktion",esc(primary.role)], ...((primary.mobile||primary.phone)?[['Mobil / Telefon',telLink(primary.mobile||primary.phone)]]:[]), ...(primary.email?[['E-Mail',mailLink(primary.email)]]:[])])}</dl></article>
  </div></section>`;
}
...
function showPlantDashboard(page){
  ...
  $("#openNavigation")?.addEventListener("click",()=>{const url=googleMapsUrls(plant).navigate;if(locationQuery(plant))window.open(url,"_blank","noopener");else alert("Bitte zuerst eine Adresse oder GPS-Koordinaten hinterlegen.");});
  ...
  $("#quickActionForm")?.addEventListener("submit",e=>{e.preventDefault();const fd=new FormData(e.currentTarget),title=String(fd.get("title")||"").trim();if(!title)return;plant.actions=[...(plant.actions||[]),{id:makeId(),title,status:"open",priority:fd.get("priority")||"normal",dueDate:fd.get("dueDate")||"",component:"",sourceVisitId:"",createdAt:new Date().toISOString(),completedAt:"",autoGenerated:false,followUpType:"",followUpSourceId:"",taskType:fd.get("taskType")||"general",contactName:fd.get("contactName")||""}];if(savePlants())showPlantDashboard("tasks");});
  $$(`[data-toggle-action]`).forEach(b=>b.onclick=()=>{const a=(plant.actions||[]).find(x=>x.id===b.dataset.toggleAction);if(!a)return;const wasDone=a.status==="done";a.status=a.status==="done"?"open":"done";a.completedAt=a.status==="done"?new Date().toISOString():"";if(wasDone)a.completionOutcome="";if(!savePlants())return;showPlantDashboard("tasks");if(a.status==="done"&&!wasDone)showCompletionFollowUpDialog({plant,sourceType:"task",source:a});});
